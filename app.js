...
function navigate(page){
  ...
  $("page-title").textContent={dashboard:"Dashboard",clientes:"Clientes",materiais:"Materiais","lista-materiais":"Lista de Materiais",servicos:"Serviços",estoque:"Estoque","reposicao-estoque":"Reposição de Estoque",orcamentos:"Orçamentos",os:"Ordens de Serviço",agenda:"Agenda",financeiro:"Financeiro",tecnicos:"Técnicos",fornecedores:"Fornecedores",compras:"Compras",relatorios:"Relatórios",recibos:"Recibos",cargos:"Cargos e Permissões","admin-plataforma":"Empresas e Licenças",unifilar:"Projeto Elétrico",usuarios:"Usuários",configuracoes:"Configurações"}[page];
...
/* =========================================================
   V044 — PROJETO ELÉTRICO E LISTA DE MATERIAIS SEM VALORES
   Substitui integralmente o editor gráfico unifilar por cadastro
   estruturado da obra, pavimentos, unidades, quadros e componentes.
   ========================================================= */
...
async function renderUnifilar(){
  ...
  $('oe-client').textContent=clientes.find(c=>c.id===obraEletricaAtual.cliente_id)?.nome||'-';
  $('oe-floor-count').textContent=obraEletricaPavimentos.length;
  $('oe-item-count').textContent=obraEletricaComponentes.length;
  const opts=obraEletricaAtual.opcoes||{};document.querySelectorAll('[data-oe-option]').forEach(x=>x.checked=!!opts[x.dataset.oeOption]);
...
async function novaObraEletrica(){
 openModal('Nova obra elétrica',`<form id="oe-new-form"><div class="form-grid"><label>Nome da obra*<input id="oen-name" required></label><label>Cliente<select id="oen-client"><option value="">Sem cliente</option>${clientes.map(c=>`<option value="${c.id}">${esc(c.nome)}</option>`).join('')}</select></label><label class="span-2">Observações<textarea id="oen-obs"></textarea></label></div><div class="modal-actions"><button type="button" class="btn secondary" onclick="closeModal()">Cancelar</button><button class="btn primary">Criar</button></div></form>`);
 $('oe-new-form').onsubmit=async e=>{e.preventDefault();const payload={empresa_id:currentUserProfile.empresa_id,nome:$('oen-name').value.trim(),cliente_id:$('oen-client').value||null,observacoes:$('oen-obs').value.trim()||null,opcoes:{}};const {data,error}=await sb.from('obras_eletricas').insert(payload).select().single();if(error)return toast(error.message);obraEletricaAtual=data;obraEletricaPavimentos=[];obraEletricaEstruturas=[];obraEletricaComponentes=[];closeModal();renderUnifilar();toast('Obra elétrica criada')}
}
async function abrirObraEletrica(){const {data,error}=await sb.from('obras_eletricas').select('*').order('updated_at',{ascending:false});if(error)return toast(error.message);openModal('Abrir obra elétrica',`<div class="license-list">${(data||[]).map(o=>`<div class="license-row"><span><b>${esc(o.nome)}</b><br><small>${new Date(o.updated_at).toLocaleString('pt-BR')}</small></span><button class="action-btn" onclick="carregarObraEletrica('${o.id}')">Abrir</button></div>`).join('')||'<p>Nenhuma obra elétrica cadastrada.</p>'}</div>`)}
...
async function salvarOpcoesObra(){if(!obraEletricaAtual)return;const opcoes={};document.querySelectorAll('[data-oe-option]').forEach(x=>opcoes[x.dataset.oeOption]=x.checked);obraEletricaAtual.opcoes=opcoes;const {error}=await sb.from('obras_eletricas').update({opcoes,updated_at:new Date().toISOString()}).eq('id',obraEletricaAtual.id);if(error)return toast(error.message);toast('Obra salva')}
function adicionarPavimento(){if(!obraEletricaAtual)return toast('Crie ou abra uma obra.');openModal('Adicionar pavimento',`<form id="oe-floor-form"><div class="form-grid"><label>Nome*<input id="oef-name" required placeholder="Ex.: Térreo, 1º Pavimento, Garagem G1"></label><label>Ordem<input id="oef-order" type="number" value="${obraEletricaPavimentos.length+1}"></label></div><div class="modal-actions"><button type="button" class="btn secondary" onclick="closeModal()">Cancelar</button><button class="btn primary">Adicionar</button></div></form>`);$('oe-floor-form').onsubmit=async e=>{e.preventDefault();const {data,error}=await sb.from('obra_eletrica_pavimentos').insert({empresa_id:currentUserProfile.empresa_id,obra_id:obraEletricaAtual.id,nome:$('oef-name').value.trim(),ordem:Number($('oef-order').value)||0}).select().single();if(error)return toast(error.message);obraEletricaPavimentos.push(data);closeModal();renderUnifilar()}}
function adicionarEstrutura(tipoInicial='quadro'){if(!obraEletricaAtual)return toast('Crie ou abra uma obra.');if(!obraEletricaPavimentos.length)return toast('Cadastre primeiro um pavimento.');const tipos=['apartamento','quadro_medicao','qgbt','subestacao','quadro_condominio','lazer','garagem','gerador','barramento_blindado','quadro','outro'];openModal(tipoInicial==='apartamento'?'Adicionar apartamento':'Adicionar quadro / equipamento',`<form id="oe-struct-form"><div class="form-grid"><label>Tipo<select id="oes-type">${tipos.map(t=>`<option value="${t}" ${t===tipoInicial?'selected':''}>${t.replaceAll('_',' ')}</option>`).join('')}</select></label><label>Identificação*<input id="oes-name" required placeholder="Ex.: Apto 101, QD-101, QGBT"></label><label>Pavimento<select id="oes-floor">${obraEletricaPavimentos.map(p=>`<option value="${p.id}">${esc(p.nome)}</option>`).join('')}</select></label><label>Sistema<select id="oes-system"><option>Monofásico</option><option>Bifásico</option><option>Trifásico</option></select></label><label class="span-2">Observações<textarea id="oes-obs"></textarea></label></div><div class="modal-actions"><button type="button" class="btn secondary" onclick="closeModal()">Cancelar</button><button class="btn primary">Adicionar</button></div></form>`);$('oe-struct-form').onsubmit=async e=>{e.preventDefault();const {data,error}=await sb.from('obra_eletrica_estruturas').insert({empresa_id:currentUserProfile.empresa_id,obra_id:obraEletricaAtual.id,pavimento_id:$('oes-floor').value,tipo:$('oes-type').value,nome:$('oes-name').value.trim(),sistema:$('oes-system').value,observacoes:$('oes-obs').value.trim()||null,ordem:obraEletricaEstruturas.length}).select().single();if(error)return toast(error.message);obraEletricaEstruturas.push(data);closeModal();renderUnifilar()}}
function adicionarComponente(){if(!obraEletricaEstruturas.length)return toast('Cadastre primeiro um apartamento, quadro ou equipamento.');const tipos=['cabo_entrada','cabo_saida','disjuntor','idr','dps','barramento_pente','barramento_neutro','barramento_terra','medidor','transformador','gerador','outro'];openModal('Adicionar componente',`<form id="oe-comp-form"><div class="form-grid"><label>Local / quadro<select id="oec-struct">${obraEletricaEstruturas.map(s=>`<option value="${s.id}">${esc(s.nome)}</option>`).join('')}</select></label><label>Tipo<select id="oec-type">${tipos.map(t=>`<option value="${t}">${t.replaceAll('_',' ')}</option>`).join('')}</select></label><label class="span-2">Material do banco*<select id="oec-material" required>${oeMaterialOptions()}</select></label><label>Quantidade<input id="oec-qtd" type="number" min="0.01" step="0.01" value="1"></label><label>Identificação<input id="oec-label" placeholder="Ex.: DJ Geral, IDR-01"></label><label class="span-2">Observações técnicas<input id="oec-obs" placeholder="Marca, modelo, cor, corrente, seção, comprimento..."></label></div><div class="modal-actions"><button type="button" class="btn secondary" onclick="closeModal()">Cancelar</button><button class="btn primary">Adicionar</button></div></form>`);enhanceAllSelects($('modal-body'));$('oe-comp-form').onsubmit=async e=>{e.preventDefault();const sid=$('oec-struct').value,s=obraEletricaEstruturas.find(x=>x.id===sid);const {data,error}=await sb.from('obra_eletrica_componentes').insert({empresa_id:currentUserProfile.empresa_id,obra_id:obraEletricaAtual.id,pavimento_id:s.pavimento_id,estrutura_id:sid,tipo:$('oec-type').value,material_id:$('oec-material').value,quantidade:Number($('oec-qtd').value)||1,identificacao:$('oec-label').value.trim()||null,observacoes:$('oec-obs').value.trim()||null,ordem:obraEletricaComponentes.length}).select().single();if(error)return toast(error.message);obraEletricaComponentes.push(data);closeModal();renderUnifilar()}}
function renderObraEletricaPavimentos(){const el=$('oe-floors');if(!el)return;el.innerHTML=obraEletricaPavimentos.map(p=>{const ss=obraEletricaEstruturas.filter(s=>s.pavimento_id===p.id);return `<section class="oe-floor"><header><div><small>PAVIMENTO</small><h3>${esc(p.nome)}</h3></div><span>${ss.length} cadastro(s)</span></header><div class="oe-struct-grid">${ss.map(s=>{const cc=obraEletricaComponentes.filter(c=>c.estrutura_id===s.id);return `<article class="oe-struct"><div class="oe-struct-head"><div><small>${esc(s.tipo.replaceAll('_',' ').toUpperCase())}</small><b>${esc(s.nome)}</b></div><span>${esc(s.sistema||'')}</span></div><div class="oe-components">${cc.map(c=>{const m=materiais.find(x=>x.id===c.material_id);return `<div class="oe-component"><span><b>${esc(c.identificacao||c.tipo.replaceAll('_',' '))}</b><small>${esc(m?.nome||'Material não encontrado')}${c.observacoes?'<br>'+esc(c.observacoes):''}</small></span><strong>${Number(c.quantidade).toLocaleString('pt-BR')} ${esc(m?.unidade||'un')}</strong></div>`}).join('')||'<span class="muted">Nenhum componente.</span>'}</div></article>`}).join('')||'<p class="muted">Nenhum quadro/equipamento neste pavimento.</p>'}</div></section>`}).join('')||'<div class="oe-empty"><b>Cadastre o primeiro pavimento.</b></div>'}
...
async function gerarListaMateriaisObra(){if(!obraEletricaAtual)return toast('Abra uma obra elétrica.');const rows=materiaisDaObra();if(!rows.length)return toast('A obra ainda não possui materiais vinculados.');const {data:l,error}=await sb.from('listas_materiais').insert({nome:`Materiais — ${obraEletricaAtual.nome}`,data_lista:today(),cliente_id:obraEletricaAtual.cliente_id||null,responsavel_emissao:currentUserProfile?.nome||currentSession?.user?.email||'',tipo_vinculo:'sem_vinculo',observacoes:`Gerada automaticamente a partir da obra elétrica: ${obraEletricaAtual.nome}`,total:0}).select().single();if(error)return toast(error.message);const payload=rows.map((r,n)=>{const m=materiais.find(x=>x.id===r.material_id);return {lista_id:l.id,material_id:r.material_id,codigo:m?.codigo||null,descricao:m?.nome||'Material',categoria:m?.categoria||null,unidade:m?.unidade||'un',quantidade:r.quantidade,valor_unitario:0,observacoes:r.observacoes||null,ordem:n,topico:'Materiais da obra'}});const ins=await sb.from('lista_materiais_itens').insert(payload);if(ins.error)return toast(ins.error.message);await loadListasMateriais();toast('Lista de materiais gerada sem valores.')}
function esquemaPavimentosHTML(){return obraEletricaPavimentos.map(p=>{const ss=obraEletricaEstruturas.filter(s=>s.pavimento_id===p.id);return `<section class="floor-page"><h2>${esc(p.nome)}</h2>${ss.map(s=>{const cc=obraEletricaComponentes.filter(c=>c.estrutura_id===s.id);return `<div class="schematic-box"><h3>${esc(s.nome)} <small>${esc(s.tipo.replaceAll('_',' '))} · ${esc(s.sistema||'')}</small></h3>${cc.map(c=>{const m=materiais.find(x=>x.id===c.material_id);return `<div class="schematic-line"><span class="dot"></span><b>${esc(c.identificacao||c.tipo.replaceAll('_',' '))}</b><span>${esc(m?.nome||'')}</span><strong>${Number(c.quantidade).toLocaleString('pt-BR')} ${esc(m?.unidade||'un')}</strong></div>`}).join('')||'<p>Sem componentes.</p>'}</div>`}).join('')||'<p>Sem quadros/equipamentos.</p>'}</section>`}).join('')}
function imprimirEsquemaPavimentos(){if(!obraEletricaAtual)return toast('Abra uma obra elétrica.');const w=window.open('','_blank');w.document.write(`<html><head><title>${esc(obraEletricaAtual.nome)}</title><style>@page{size:A4 landscape;margin:10mm}body{font:11px Arial;color:#111}.floor-page{break-after:page;page-break-after:always}.floor-page:last-child{break-after:auto}.schematic-box{border:1px solid #222;margin:8px 0;padding:8px;break-inside:avoid}.schematic-box h3{margin:0 0 8px}.schematic-box small{font-weight:normal}.schematic-line{display:grid;grid-template-columns:16px 150px 1fr 100px;gap:8px;align-items:center;border-top:1px solid #ddd;padding:6px}.dot{width:7px;height:7px;border-radius:50%;background:#111}</style></head><body><h1>${esc(obraEletricaAtual.nome)} — Esquema por pavimentos</h1>${esquemaPavimentosHTML()}<script>onload=()=>print()<\/script></body></html>`);w.document.close()}

// LISTA DE MATERIAIS V044: tópicos, drag & drop, sem valores.
function listaMaterialForm(lista=null,itens=[]){
 listaMaterialItens=(itens||[]).map((i,n)=>({material_id:i.material_id,codigo:i.codigo||'',descricao:i.descricao||'',categoria:i.categoria||'',unidade:i.unidade||'un',quantidade:Number(i.quantidade)||1,observacoes:i.observacoes||'',topico:i.topico||'Geral',ordem:Number(i.ordem??n)}));
 listaMaterialTopicos=[...new Set(listaMaterialItens.map(i=>i.topico||'Geral'))];if(!listaMaterialTopicos.length)listaMaterialTopicos=['Geral'];const tipo=lista?.tipo_vinculo||'sem_vinculo';
 openModal(lista?.id?'Editar Lista de Materiais':'Nova Lista de Materiais',`<form id="lista-material-form"><div class="form-grid"><label>Nome da lista*<input id="lm-nome" required value="${esc(lista?.nome||'')}"></label><label>Data*<input id="lm-data" type="date" required value="${lista?.data_lista||today()}"></label><label>Cliente<select id="lm-cliente"><option value="">Selecione...</option>${clientes.map(c=>`<option value="${c.id}" ${c.id===lista?.cliente_id?'selected':''}>${esc(c.nome)}</option>`).join('')}</select></label><label>Responsável<input id="lm-responsavel" value="${esc(lista?.responsavel_emissao||currentUserProfile?.nome||'')}"></label><label>Vínculo<select id="lm-tipo"><option value="sem_vinculo">Sem vínculo</option><option value="orcamento">Orçamento</option><option value="recibo">Recibo</option><option value="ordem_servico">OS</option></select></label><label class="span-2">Observações<textarea id="lm-obs">${esc(lista?.observacoes||'')}</textarea></label></div><div class="lm-topic-toolbar"><button type="button" class="btn secondary" id="lm-add-topic">+ Tópico</button><select id="lm-topic-select"></select><select id="lm-material">${materiais.map(m=>`<option value="${m.id}">${esc(m.codigo||'')} — ${esc(m.nome)}</option>`).join('')}</select><input id="lm-qtd" type="number" min="0.01" step="0.01" value="1"><input id="lm-item-obs" placeholder="Marca, modelo, cor, observações..."><button type="button" class="btn primary" id="lm-add">Adicionar</button></div><div id="lm-topic-editor" class="lm-topic-editor"></div><div class="modal-actions"><button type="button" class="btn secondary" onclick="closeModal()">Cancelar</button><button class="btn primary">Salvar Lista</button></div></form>`);
 $('lm-tipo').value=tipo;const refreshTopics=()=>{$('lm-topic-select').innerHTML=listaMaterialTopicos.map(t=>`<option>${esc(t)}</option>`).join('')};refreshTopics();$('lm-add-topic').onclick=()=>{const n=prompt('Nome do tópico:');if(n&& !listaMaterialTopicos.includes(n.trim())){listaMaterialTopicos.push(n.trim());refreshTopics();renderListaMaterialItens()}};$('lm-add').onclick=addListaMaterialItem;$('lista-material-form').onsubmit=e=>saveListaMaterial(e,lista?.id||null);renderListaMaterialItens();enhanceAllSelects($('modal-body'));
}
function addListaMaterialItem(){const id=$('lm-material').value,q=Number($('lm-qtd').value),m=materiais.find(x=>x.id===id);if(!m||q<=0)return toast('Selecione um material e informe a quantidade.');listaMaterialItens.push({material_id:m.id,codigo:m.codigo||'',descricao:m.nome,categoria:m.categoria||'',unidade:m.unidade||'un',quantidade:q,observacoes:$('lm-item-obs').value.trim(),topico:$('lm-topic-select').value||'Geral',ordem:listaMaterialItens.length});$('lm-qtd').value=1;$('lm-item-obs').value='';renderListaMaterialItens()}
function renderListaMaterialItens(){const box=$('lm-topic-editor');if(!box)return;box.innerHTML=listaMaterialTopicos.map((t,ti)=>`<section class="lm-topic" draggable="true" data-topic="${esc(t)}"><header><span class="drag-handle">⋮⋮</span><b>${esc(t)}</b><div><button type="button" class="action-btn" onclick="moveLmTopic(${ti},-1)">↑</button><button type="button" class="action-btn" onclick="moveLmTopic(${ti},1)">↓</button></div></header><div class="lm-topic-items">${listaMaterialItens.filter(i=>(i.topico||'Geral')===t).map(i=>{const n=listaMaterialItens.indexOf(i);return `<div class="lm-item" draggable="true" data-index="${n}"><span class="drag-handle">⋮⋮</span><div class="lm-item-desc"><b>${esc(i.descricao)}</b>${i.observacoes?`<small>${esc(i.observacoes)}</small>`:''}</div><span>${Number(i.quantidade).toLocaleString('pt-BR')} ${esc(i.unidade)}</span><select onchange="listaMaterialItens[${n}].topico=this.value;renderListaMaterialItens()">${listaMaterialTopicos.map(x=>`<option ${x===t?'selected':''}>${esc(x)}</option>`).join('')}</select><button type="button" class="action-btn danger" onclick="removeListaMaterialItem(${n})">Excluir</button></div>`}).join('')||'<p class="muted">Arraste materiais para este tópico.</p>'}</div></section>`).join('');enableLmDrag()}
...
async function viewListaMaterial(id){const x=await getListaMaterial(id);if(!x)return;const groups=[...new Set(x.itens.map(i=>i.topico||'Geral'))];openModal(x.lista.nome,`${groups.map(t=>`<section class="lm-view-topic"><h4>${esc(t)}</h4>${x.itens.filter(i=>(i.topico||'Geral')===t).map(i=>`<div class="lm-view-row"><div><b>${esc(i.descricao)}</b>${i.observacoes?`<small>${esc(i.observacoes)}</small>`:''}</div><span>${Number(i.quantidade).toLocaleString('pt-BR')} ${esc(i.unidade)}</span></div>`).join('')}</section>`).join('')}<div class="modal-actions"><button class="btn secondary" onclick="closeModal()">Fechar</button><button class="btn primary" onclick="printListaMaterial('${id}')">PDF / Imprimir</button></div>`)}
async function printListaMaterial(id){const x=await getListaMaterial(id);if(!x)return;const l=x.lista,topics=(l.topicos?.length?l.topicos:[...new Set(x.itens.map(i=>i.topico||'Geral'))]);const w=window.open('','_blank');w.document.write(`<html><head><title>${esc(l.nome)}</title><style>@page{size:A4;margin:12mm}body{font:11px Arial;color:#111}h1{font-size:18px}.topic{break-inside:avoid;margin:14px 0}.topic h2{font-size:13px;border-bottom:1px solid #222;padding-bottom:4px}.row{display:grid;grid-template-columns:1fr 130px;border-bottom:1px solid #bbb;min-height:46px;break-inside:avoid}.mat{padding:7px 9px}.mat b{display:block;font-size:12px}.mat small{display:block;font-size:9px;font-style:italic;margin-top:3px}.qty{border-left:1px solid #bbb;display:flex;align-items:center;justify-content:center;font-size:11px}.head{display:grid;grid-template-columns:1fr 130px;border:1px solid #222;text-align:center;padding:4px 0}.head span+span{border-left:1px solid #222}.footer{margin-top:15px;font-size:9px;color:#555}</style></head><body><h1>${esc(l.nome)}</h1>${clientePDFHTML(l.clientes)}${topics.map(t=>`<section class="topic"><h2>${esc(t)}</h2><div class="head"><span>MATERIAL</span><span>QUANTIDADE</span></div>${x.itens.filter(i=>(i.topico||'Geral')===t).map(i=>`<div class="row"><div class="mat"><b>${esc(i.descricao)}</b>${i.observacoes?`<small>${esc(i.observacoes).replaceAll('\n','<br>')}</small>`:''}</div><div class="qty">${Number(i.quantidade).toLocaleString('pt-BR')} ${esc(i.unidade)}</div></div>`).join('')}</section>`).join('')}<div class="footer">${esc(l.observacoes||'')}</div><script>onload=()=>print()<\/script></body></html>`);w.document.close()}
...
// Bind do novo módulo
setTimeout(()=>{if($('oe-new'))$('oe-new').onclick=novaObraEletrica;if($('oe-open'))$('oe-open').onclick=abrirObraEletrica;if($('oe-add-floor'))$('oe-add-floor').onclick=adicionarPavimento;if($('oe-add-apartment'))$('oe-add-apartment').onclick=()=>adicionarEstrutura('apartamento');if($('oe-add-board'))$('oe-add-board').onclick=()=>adicionarEstrutura('quadro');if($('oe-add-component'))$('oe-add-component').onclick=adicionarComponente;if($('oe-save'))$('oe-save').onclick=salvarOpcoesObra;if($('oe-materials'))$('oe-materials').onclick=gerarListaMateriaisObra;if($('oe-pdf'))$('oe-pdf').onclick=imprimirEsquemaPavimentos},0);
