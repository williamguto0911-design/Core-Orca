...
function adicionarComponente(c0=null){if(!obraEletricaEstruturas.length)return toast('Cadastre primeiro um apartamento, quadro ou equipamento.');const tipos=['cabo_entrada','cabo_saida','disjuntor','idr','dps','barramento_pente','barramento_neutro','barramento_terra','medidor','transformador','gerador','outro'];openModal(c0?'Editar componente':'Adicionar componente',`<form id="oe-comp-form"><div class="form-grid"><label>Local / quadro<select id="oec-struct">${obraEletricaEstruturas.map(s=>`<option value="${s.id}" ${s.id===c0?.estrutura_id?'selected':''}>${esc(s.nome)}</option>`).join('')}</select></label><label>Tipo<select id="oec-type">${tipos.map(t=>`<option value="${t}" ${t===c0?.tipo?'selected':''}>${t.replaceAll('_',' ')}</option>`).join('')}</select></label><label class="span-2">Material do banco*<select id="oec-material" required>${oeMaterialOptions(c0?.material_id)}</select></label><label>Quantidade<input id="oec-qtd" type="number" min="0.01" step="0.01" value="${c0?.quantidade??1}"></label><label>Identificação<input id="oec-label" value="${esc(c0?.identificacao||'')}" placeholder="Ex.: DJ Geral, IDR-01"></label><label class="span-2">Observações técnicas<input id="oec-obs" value="${esc(c0?.observacoes||'')}" placeholder="Marca, modelo, cor, corrente, seção, comprimento..."></label></div><div class="modal-actions"><button type="button" class="btn secondary" onclick="closeModal()">Cancelar</button><button class="btn primary">Salvar</button></div></form>`);enhanceAllSelects($('modal-body'));$('oe-comp-form').onsubmit=async e=>{e.preventDefault();const sid=$('oec-struct').value,s=obraEletricaEstruturas.find(x=>x.id===sid);const payload={pavimento_id:s.pavimento_id,estrutura_id:sid,tipo:$('oec-type').value,material_id:$('oec-material').value,quantidade:Number($('oec-qtd').value)||1,identificacao:$('oec-label').value.trim()||null,observacoes:$('oec-obs').value.trim()||null};let r=c0?await sb.from('obra_eletrica_componentes').update(payload).eq('id',c0.id).select().single():await sb.from('obra_eletrica_componentes').insert({...payload,empresa_id:currentUserProfile.empresa_id,obra_id:obraEletricaAtual.id,ordem:obraEletricaComponentes.length}).select().single();if(r.error)return toast(r.error.message);if(c0)obraEletricaComponentes=obraEletricaComponentes.map(x=>x.id===c0.id?r.data:x);else obraEletricaComponentes.push(r.data);closeModal();renderUnifilar()}}
...
// Bind do novo módulo
setTimeout(()=>{if($('oe-new'))$('oe-new').onclick=novaObraEletrica;if($('oe-open'))$('oe-open').onclick=abrirObraEletrica;if($('oe-add-floor'))$('oe-add-floor').onclick=adicionarPavimento;if($('oe-add-apartment'))$('oe-add-apartment').onclick=()=>adicionarEstrutura('apartamento');if($('oe-add-board'))$('oe-add-board').onclick=()=>adicionarEstrutura('quadro');if($('oe-add-component'))$('oe-add-component').onclick=adicionarComponente;if($('oe-edit-work'))$('oe-edit-work').onclick=editarObraEletrica;if($('oe-save'))$('oe-save').onclick=salvarOpcoesObra;if($('oe-materials'))$('oe-materials').onclick=gerarListaMateriaisObra;if($('oe-pdf'))$('oe-pdf').onclick=imprimirEsquemaPavimentos},0);
