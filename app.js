...
let lmEditingIndex=-1, editorEditingIndex=-1;
...
async function loadCurrentProfile(user){
 ...
 try{
   const {data:vinculo,error}=await sb.from("empresa_usuarios")
     .select("id,empresa_id,nome,email,tipo,cargo_id,ativo,deve_trocar_senha")
     .eq("auth_user_id",user.id).eq("ativo",true).limit(1).maybeSingle();
   if(error)throw error;
   if(vinculo){
     let cargoPermissions=rpcData?.permissions||{};
     if(vinculo.tipo!=="gerente" && vinculo.cargo_id){
       try{
         const {data:perms,error:pe}=await sb.rpc("meu_cargo_permissoes_v035");
         if(pe)console.warn("Permissões do cargo:",pe.message);
         else if(perms && typeof perms==="object")cargoPermissions=perms;
       }catch(e){console.warn("Permissões do cargo:",e)}
     }
     currentUserProfile={
...
function reciboVinculoOptions(tipo,selected=""){
 const src=tipo==="orcamento"?orcamentos:tipo==="lista_materiais"?listasMateriais:tipo==="ordem_servico"?ordens:[];
 return '<option value="">Selecione...</option>'+src.map(x=>{const label=tipo==="lista_materiais"?(x.nome||"Lista de materiais"):(x.numero||x.nome||"");const cli=x.clientes?.nome||clientes.find(c=>c.id===x.cliente_id)?.nome||"";return `<option value="${x.id}" ${x.id===selected?"selected":""}>${esc(label)}${cli?` — ${esc(cli)}`:""}</option>`}).join("");
}
function reciboVinculoCliente(tipo,id){
 const src=tipo==="orcamento"?orcamentos:tipo==="lista_materiais"?listasMateriais:tipo==="ordem_servico"?ordens:[];
 return src.find(x=>String(x.id)===String(id))?.cliente_id||null;
}
async function carregarItensVinculoRecibo(tipo,id){
 if(!tipo||tipo==="sem_vinculo"||!id){editorItens=[];renderEditorItens();return 0}
 let table,fk;
 if(tipo==="orcamento"){table="orcamento_itens";fk="orcamento_id"}
 else if(tipo==="lista_materiais"){table="lista_materiais_itens";fk="lista_id"}
 else if(tipo==="ordem_servico"){table="ordem_servico_itens";fk="ordem_servico_id"}
 else return 0;
 const {data,error}=await sb.from(table).select("*").eq(fk,id).order("ordem");
 if(error){toast("Erro ao carregar itens do vínculo: "+error.message);return -1}
 editorItens=(data||[]).map(i=>{
   if(tipo==="lista_materiais") return {tipo:"material",referencia_id:i.material_id||null,descricao:i.descricao||"Material",quantidade:Number(i.quantidade)||1,valor_unitario:Number(i.valor_unitario)||0};
   return {tipo:i.tipo|| (i.material_id?"material":"servico"),referencia_id:i.material_id||i.servico_id||null,descricao:i.descricao||"Item",quantidade:Number(i.quantidade)||1,valor_unitario:Number(i.valor_unitario)||0};
 });
 renderEditorItens();
 return editorItens.length;
}
function reciboForm(){
 editorItens=[];let pagamentos=[];
 openModal("Novo recibo",`<form id="entity-form"><div class="form-grid"><label>Cliente*<select id="f-cliente" required><option value="">Selecione...</option>${clientes.map(c=>`<option value="${c.id}">${esc(c.nome)}</option>`).join("")}</select></label><label>Emissão<input id="f-data" type="date" value="${today()}"></label><label>Tipo de vínculo<select id="f-vinc-tipo"><option value="sem_vinculo">Sem vínculo</option><option value="orcamento">Orçamento</option><option value="lista_materiais">Lista de Materiais</option><option value="ordem_servico">Ordem de Serviço (OS)</option></select></label><label>Documento vinculado<select id="f-vinc-id" disabled><option value="">Selecione o tipo de vínculo...</option></select></label><label class="span-2">Referente a / observações<textarea id="f-obs"></textarea></label></div>${itemEditorHTML()}
 <div class="item-builder"><div class="panel-head"><b>Resumo financeiro</b></div><div class="summary-grid"><div class="summary-card"><span>Total do recibo</span><b id="new-r-total">${money(0)}</b></div><div class="summary-card"><span>Pagamentos lançados</span><b id="new-r-paid">${money(0)}</b></div><div class="summary-card"><span>Saldo a parcelar</span><b id="new-r-balance">${money(0)}</b></div></div></div>
 <div class="item-builder"><div class="panel-head"><b>Lançar pagamento</b></div><div class="form-grid"><label>Data do pagamento<input id="new-pay-date" type="date" value="${today()}"></label><label>Valor<input id="new-pay-value" type="number" min="0.01" step="0.01"></label><label>Forma de pagamento<select id="new-pay-method"><option value="">Selecione...</option><option>PIX</option><option>Dinheiro</option><option>Cartão de crédito</option><option>Cartão de débito</option><option>Boleto</option><option>Transferência</option><option>Cheque</option><option>Outro</option></select></label><label>Observação<input id="new-pay-note" placeholder="Observação do pagamento"></label></div><button type="button" id="new-pay-add" class="btn primary">Lançar pagamento</button></div>
 <div class="item-builder"><div class="panel-head"><b>Parcelar saldo restante</b></div><div class="form-grid"><label>Nº de parcelas<input id="auto-n" type="number" min="1" value="1"></label><label>Data da primeira parcela<input id="auto-first" type="date" value="${today()}"></label><label>Intervalo<select id="auto-int"><option value="30">Mensal (30 dias)</option><option value="15">15 dias</option><option value="7">Semanal</option></select></label><label>Forma de pagamento<select id="auto-method"><option value="">Selecione...</option><option>PIX</option><option>Dinheiro</option><option>Cartão de crédito</option><option>Cartão de débito</option><option>Boleto</option><option>Transferência</option><option>Cheque</option><option>Outro</option></select></label></div><button type="button" id="auto-split" class="btn secondary">Gerar parcelamento do saldo restante</button></div>
 <div class="item-builder"><div class="panel-head"><b>Pagamentos e parcelas</b></div><div id="payments-box"></div></div><div class="modal-actions"><button type="button" class="btn secondary" onclick="closeModal()">Cancelar</button><button class="btn primary">Salvar recibo</button></div></form>`);
 setupItemEditor();
 const totalRecibo=()=>Math.round(editorItens.reduce((s,i)=>s+Number(i.quantidade||0)*Number(i.valor_unitario||0),0)*100)/100;
 const totalPago=()=>Math.round(pagamentos.filter(p=>p.status==="pago").reduce((s,p)=>s+Number(p.valor||0),0)*100)/100;
 const saldoRestante=()=>Math.max(0,Math.round((totalRecibo()-totalPago())*100)/100);
 const updateSummary=()=>{if($("new-r-total"))$("new-r-total").textContent=money(totalRecibo());if($("new-r-paid"))$("new-r-paid").textContent=money(totalPago());if($("new-r-balance"))$("new-r-balance").textContent=money(saldoRestante());};
 const draw=()=>{const b=$("payments-box");b.innerHTML=pagamentos.length?pagamentos.map((p,n)=>`<div class="payment-row"><label>${p.status==="pago"?"Pagamento":"Vencimento"}<input data-pn="${n}" data-pk="data" type="date" value="${p.data||""}"></label><label>Valor<input data-pn="${n}" data-pk="valor" type="number" min="0" step="0.01" value="${p.valor||0}"></label><label>Forma<input data-pn="${n}" data-pk="metodo" value="${esc(p.metodo||"")}"></label><label>Status<input value="${p.status==="pago"?"Pago":"Pendente"}" disabled></label><button type="button" class="action-btn danger" data-prm="${n}">Excluir</button></div>`).join(""):`<p class="muted">Nenhum pagamento ou parcela lançado.</p>`;b.querySelectorAll("[data-pn]").forEach(i=>i.onchange=()=>{pagamentos[Number(i.dataset.pn)][i.dataset.pk]=i.dataset.pk==="valor"?Number(i.value):i.value;updateSummary()});b.querySelectorAll("[data-prm]").forEach(x=>x.onclick=()=>{pagamentos.splice(Number(x.dataset.prm),1);draw();updateSummary()});updateSummary()};
 const oldRender=renderEditorItens; // mantém o editor visual atual
 $("f-vinc-tipo").onchange=async()=>{const tipo=$("f-vinc-tipo").value,sel=$("f-vinc-id");editorItens=[];renderEditorItens();pagamentos=pagamentos.filter(p=>p.status==="pago");if(tipo==="sem_vinculo"){sel.innerHTML='<option value="">Sem vínculo</option>';sel.disabled=true;draw();return}sel.disabled=false;sel.innerHTML=reciboVinculoOptions(tipo);enhanceAllSelects($("modal-body"));draw()};
 $("f-vinc-id").onchange=async()=>{const tipo=$("f-vinc-tipo").value,id=$("f-vinc-id").value;if(!id)return;const cli=reciboVinculoCliente(tipo,id);if(cli)$("f-cliente").value=cli;const n=await carregarItensVinculoRecibo(tipo,id);if(n>=0){pagamentos=pagamentos.filter(p=>p.status==="pago");draw();toast(`${n} item(ns) carregado(s) automaticamente.`)}};
 $("new-pay-add").onclick=()=>{const valor=Number($("new-pay-value").value||0),data=$("new-pay-date").value,metodo=$("new-pay-method").value,obs=$("new-pay-note").value.trim(),saldo=saldoRestante();if(!data||!metodo||valor<=0)return toast("Informe data, valor e forma de pagamento.");if(valor>saldo+0.01)return toast("O pagamento não pode ser maior que o saldo restante.");pagamentos.push({data,valor,status:"pago",metodo,observacao:obs});$("new-pay-value").value="";$("new-pay-note").value="";draw();toast("Pagamento lançado. O saldo restante foi atualizado.")};
 $("auto-split").onclick=()=>{const total=totalRecibo(),pago=totalPago(),saldo=Math.max(0,Math.round((total-pago)*100)/100);if(total<=0)return toast("Adicione os itens do recibo antes de parcelar.");if(saldo<=0)return toast("Não existe saldo restante para parcelar.");const n=Math.max(1,Number($("auto-n").value)||1),days=Number($("auto-int").value)||30,first=$("auto-first").value,method=$("auto-method").value;if(!first||!method)return toast("Informe a primeira data e a forma de pagamento.");pagamentos=pagamentos.filter(p=>p.status==="pago");const cents=Math.round(saldo*100),baseCents=Math.floor(cents/n),rest=cents-baseCents*n;for(let i=0;i<n;i++){const d=new Date(first+"T12:00:00");d.setDate(d.getDate()+days*i);pagamentos.push({data:d.toISOString().slice(0,10),valor:(baseCents+(i===n-1?rest:0))/100,status:"pendente",metodo:method,observacao:`Parcela ${i+1}/${n}`})}draw();toast(`Parcelamento gerado somente sobre o saldo restante de ${money(saldo)}.`)};
 // Atualiza resumo ao alterar itens no editor, sem mexer na estética existente.
 $("modal-body").addEventListener("change",()=>setTimeout(updateSummary,0));
 draw();
 $("entity-form").onsubmit=async e=>{e.preventDefault();if(!editorItens.length)return toast("Adicione ao menos um item.");const total=totalRecibo(),soma=pagamentos.reduce((s,p)=>s+Number(p.valor||0),0);if(!pagamentos.length)return toast("Lance um pagamento ou gere o parcelamento do saldo restante.");if(Math.abs(soma-total)>0.02)return toast("Os pagamentos e parcelas devem completar o total do recibo. Saldo não distribuído: "+money(total-soma));const tipo=$("f-vinc-tipo").value,id=$("f-vinc-id").value||null;const header={cliente_id:$("f-cliente").value,ordem_servico_id:tipo==="ordem_servico"?id:null,orcamento_id:tipo==="orcamento"?id:null,lista_materiais_id:tipo==="lista_materiais"?id:null,tipo_vinculo:tipo,data_emissao:$("f-data").value,observacoes:$("f-obs").value.trim(),total,status:pagamentos.every(p=>p.status==="pago")?"pago":"aberto"};const {data,error}=await sb.from("recibos").insert(header).select().single();if(error)return toast(error.message);let r=await sb.from("recibo_itens").insert(editorItens.map((i,n)=>({recibo_id:data.id,tipo:i.tipo,material_id:i.tipo==="material"?i.referencia_id:null,servico_id:i.tipo==="servico"?i.referencia_id:null,descricao:i.descricao,quantidade:i.quantidade,valor_unitario:i.valor_unitario,ordem:n})));if(r.error)return toast(r.error.message);r=await sb.from("recibo_pagamentos").insert(pagamentos.map((p,n)=>({recibo_id:data.id,parcela:n+1,vencimento:p.data,data_pagamento:p.status==="pago"?p.data:null,valor:p.valor,status:p.status,metodo_pagamento:p.metodo||null,observacao:p.observacao||null})));if(r.error)return toast(r.error.message);closeModal();await loadRecibos();renderRecibos();toast("Recibo criado: "+data.numero)};
 enhanceAllSelects($("modal-body"));
}

async function editRecibo(id){
 const r=recibos.find(x=>x.id===id);if(!r)return;
 const {data:itens,error:itErr}=await sb.from("recibo_itens").select("*").eq("recibo_id",id).order("ordem");if(itErr)return toast(itErr.message);
 editorItens=(itens||[]).map(i=>({tipo:i.tipo,referencia_id:i.material_id||i.servico_id||null,descricao:i.descricao,quantidade:Number(i.quantidade),valor_unitario:Number(i.valor_unitario)}));
 const tipoAtual=r.tipo_vinculo|| (r.ordem_servico_id?"ordem_servico":r.orcamento_id?"orcamento":r.lista_materiais_id?"lista_materiais":"sem_vinculo");
 const idAtual=tipoAtual==="ordem_servico"?r.ordem_servico_id:tipoAtual==="orcamento"?r.orcamento_id:tipoAtual==="lista_materiais"?r.lista_materiais_id:"";
 openModal("Editar "+r.numero,`<form id="edit-receipt-form"><div class="form-grid"><label>Cliente<select id="er-client">${clientes.map(c=>`<option value="${c.id}">${esc(c.nome)}</option>`).join("")}</select></label><label>Emissão<input id="er-date" type="date" value="${r.data_emissao||today()}"></label><label>Tipo de vínculo<select id="er-vinc-tipo"><option value="sem_vinculo">Sem vínculo</option><option value="orcamento">Orçamento</option><option value="lista_materiais">Lista de Materiais</option><option value="ordem_servico">Ordem de Serviço (OS)</option></select></label><label>Documento vinculado<select id="er-vinc-id"></select></label><label class="span-2">Observações<textarea id="er-obs">${esc(r.observacoes||"")}</textarea></label></div>${itemEditorHTML()}<div class="modal-actions"><button type="button" class="btn secondary" onclick="closeModal()">Cancelar</button><button class="btn primary">Salvar</button></div></form>`);
 $("er-client").value=r.cliente_id;$("er-vinc-tipo").value=tipoAtual;$("er-vinc-id").innerHTML=tipoAtual==="sem_vinculo"?'<option value="">Sem vínculo</option>':reciboVinculoOptions(tipoAtual,idAtual);$("er-vinc-id").value=idAtual||"";$("er-vinc-id").disabled=tipoAtual==="sem_vinculo";setupItemEditor();
 $("er-vinc-tipo").onchange=()=>{const tipo=$("er-vinc-tipo").value,sel=$("er-vinc-id");if(tipo==="sem_vinculo"){sel.innerHTML='<option value="">Sem vínculo</option>';sel.disabled=true;editorItens=[];renderEditorItens()}else{sel.disabled=false;sel.innerHTML=reciboVinculoOptions(tipo);editorItens=[];renderEditorItens()}enhanceAllSelects($("modal-body"))};
 $("er-vinc-id").onchange=async()=>{const tipo=$("er-vinc-tipo").value,vid=$("er-vinc-id").value;if(!vid)return;const cli=reciboVinculoCliente(tipo,vid);if(cli)$("er-client").value=cli;const n=await carregarItensVinculoRecibo(tipo,vid);if(n>=0)toast(`${n} item(ns) carregado(s) automaticamente.`)};
 $("edit-receipt-form").onsubmit=async e=>{e.preventDefault();if(!editorItens.length)return toast("Adicione ao menos um item.");const total=editorItens.reduce((s,i)=>s+i.quantidade*i.valor_unitario,0),tipo=$("er-vinc-tipo").value,vid=$("er-vinc-id").value||null;const {error}=await sb.from("recibos").update({cliente_id:$("er-client").value,data_emissao:$("er-date").value,ordem_servico_id:tipo==="ordem_servico"?vid:null,orcamento_id:tipo==="orcamento"?vid:null,lista_materiais_id:tipo==="lista_materiais"?vid:null,tipo_vinculo:tipo,observacoes:$("er-obs").value.trim(),total}).eq("id",id);if(error)return toast(error.message);let q=await sb.from("recibo_itens").delete().eq("recibo_id",id);if(q.error)return toast(q.error.message);q=await sb.from("recibo_itens").insert(editorItens.map((i,n)=>({recibo_id:id,tipo:i.tipo,material_id:i.tipo==="material"?i.referencia_id:null,servico_id:i.tipo==="servico"?i.referencia_id:null,descricao:i.descricao,quantidade:i.quantidade,valor_unitario:i.valor_unitario,ordem:n})));if(q.error)return toast(q.error.message);closeModal();await loadRecibos();renderRecibos();toast("Recibo atualizado")};
 enhanceAllSelects($("modal-body"));
}
...
async function loadReciboPagamentos(reciboId){
 const {data,error}=await sb.from("recibo_pagamentos").select("*").eq("recibo_id",reciboId).order("parcela");
 if(error){toast(error.message);return null} return data||[];
}
function reciboFinanceiro(r,pags){
 const total=Number(r?.total||0),pago=(pags||[]).filter(p=>p.status==="pago").reduce((a,p)=>a+Number(p.valor||0),0);
 return {total,pago,saldo:Math.max(0,Math.round((total-pago)*100)/100),status:total>0&&pago>=total-0.01?"Pago":pago>0?"Parcial":"Pendente"};
}
async function atualizarStatusReciboFinanceiro(reciboId){
 const r=recibos.find(x=>x.id===reciboId); if(!r)return;
 const pags=await loadReciboPagamentos(reciboId); if(!pags)return;
 const f=reciboFinanceiro(r,pags),status=f.status==="Pago"?"pago":"aberto";
 await sb.from("recibos").update({status}).eq("id",reciboId); await loadRecibos();
}
async function gerenciarPagamentosRecibo(reciboId){
 const r=recibos.find(x=>x.id===reciboId); if(!r)return;
 const pags=await loadReciboPagamentos(reciboId); if(!pags)return;
 const f=reciboFinanceiro(r,pags);
 openModal("Pagamentos / parcelamento — "+r.numero,`
 <div class="summary-grid"><div class="summary-card"><span>Total do recibo</span><b>${money(f.total)}</b></div><div class="summary-card"><span>Total pago</span><b>${money(f.pago)}</b></div><div class="summary-card"><span>Saldo restante</span><b>${money(f.saldo)}</b></div><div class="summary-card"><span>Status</span><b>${f.status}</b></div></div>
 <div class="item-builder"><div class="panel-head"><b>Lançar pagamento</b></div><div class="form-grid"><label>Data do pagamento<input id="rp-data" type="date" value="${today()}"></label><label>Valor<input id="rp-valor" type="number" min="0.01" step="0.01" max="${f.saldo}" value="${f.saldo>0?f.saldo:""}"></label><label>Forma de pagamento<select id="rp-forma"><option value="">Selecione...</option><option>PIX</option><option>Dinheiro</option><option>Cartão de crédito</option><option>Cartão de débito</option><option>Boleto</option><option>Transferência</option><option>Cheque</option><option>Outro</option></select></label><label>Observação<input id="rp-obs" placeholder="Observação do pagamento"></label><label class="span-2">Comprovante<input id="rp-proof" type="file" accept="image/*,.pdf"></label></div><button type="button" class="btn primary" id="rp-add">Lançar pagamento</button></div>
 <div class="item-builder"><div class="panel-head"><b>Configurar parcelas do saldo restante</b></div><div class="form-grid"><label>Quantidade de parcelas<input id="rp-n" type="number" min="1" value="1"></label><label>Data da primeira parcela<input id="rp-first" type="date" value="${today()}"></label><label>Intervalo entre parcelas (dias)<input id="rp-int" type="number" min="1" value="30"></label><label>Forma de pagamento<select id="rp-method"><option value="">Selecione...</option><option>PIX</option><option>Dinheiro</option><option>Cartão de crédito</option><option>Cartão de débito</option><option>Boleto</option><option>Transferência</option><option>Cheque</option><option>Outro</option></select></label><label class="span-2">Observação padrão<input id="rp-note" placeholder="Ex.: Parcela"></label></div><button type="button" class="btn secondary" id="rp-split">Gerar parcelas</button></div>
 <div class="table-wrap"><table><thead><tr><th>Parcela</th><th>Vencimento</th><th>Pagamento</th><th>Valor</th><th>Forma</th><th>Status</th><th>Observação</th><th>Ações</th></tr></thead><tbody>${pags.length?pags.map(p=>`<tr><td>${p.parcela||"-"}</td><td>${p.vencimento?new Date(p.vencimento+"T12:00:00").toLocaleDateString("pt-BR"):"-"}</td><td>${p.data_pagamento?new Date(p.data_pagamento+"T12:00:00").toLocaleDateString("pt-BR"):"-"}</td><td>${money(p.valor)}</td><td>${esc(p.metodo_pagamento||"-")}</td><td>${p.status==="pago"?"Pago":"Pendente"}</td><td>${esc(p.observacao||"")}</td><td><button class="action-btn" onclick="paymentPopup('${p.id}','${reciboId}',${p.status!=="pago"})">${p.status==="pago"?"Marcar pendente":"Marcar paga"}</button> <button class="action-btn danger" onclick="removeReciboPagamento('${p.id}','${reciboId}')">Excluir</button>${p.comprovante_path?` <button class="action-btn" onclick="openProof('${p.comprovante_path}')">Comprovante</button>`:""}</td></tr>`).join(""):`<tr><td colspan="8">Nenhum pagamento registrado.</td></tr>`}</tbody></table></div>
 <div class="modal-actions"><button class="btn secondary" onclick="viewRecibo('${reciboId}')">Voltar ao recibo</button></div>`);
 $("rp-add").onclick=async()=>{
   const valor=Number($("rp-valor").value||0),forma=$("rp-forma").value,data=$("rp-data").value,obs=$("rp-obs").value.trim();
   if(!valor||valor<=0||!forma||!data)return toast("Informe data, valor e forma de pagamento.");
   if(valor>f.saldo+0.01)return toast("O pagamento não pode ser maior que o saldo restante.");
   let path=null,file=$("rp-proof").files?.[0]; if(file){const ext=file.name.split(".").pop()||"bin",np=`${currentUserProfile?.empresa_id}/${reciboId}/${crypto.randomUUID()}.${ext}`;const up=await sb.storage.from("comprovantes").upload(np,file,{contentType:file.type||"application/octet-stream"});if(up.error)return toast(up.error.message);path=np}
   const prox=(pags.reduce((m,p)=>Math.max(m,Number(p.parcela)||0),0)||0)+1;
   const {error}=await sb.from("recibo_pagamentos").insert({recibo_id:reciboId,parcela:prox,vencimento:data,data_pagamento:data,valor,status:"pago",metodo_pagamento:forma,comprovante_path:path,observacao:obs||null});if(error)return toast(error.message);
   await atualizarStatusReciboFinanceiro(reciboId); await gerenciarPagamentosRecibo(reciboId); toast("Pagamento lançado com sucesso.");
 };
 $("rp-split").onclick=async()=>{
   if(f.saldo<=0)return toast("Não existe saldo restante para parcelar.");
   const n=Math.max(1,Number($("rp-n").value)||0),first=$("rp-first").value,days=Math.max(1,Number($("rp-int").value)||30),method=$("rp-method").value,note=$("rp-note").value.trim();
   if(!n||!first||!method)return toast("Preencha quantidade, primeira data e forma de pagamento.");
   const cents=Math.round(f.saldo*100),base=Math.floor(cents/n),rest=cents-base*n,start=(pags.reduce((m,p)=>Math.max(m,Number(p.parcela)||0),0)||0),rows=[];
   for(let i=1;i<=n;i++){const d=new Date(first+"T12:00:00");d.setDate(d.getDate()+(i-1)*days);rows.push({recibo_id:reciboId,parcela:start+i,vencimento:d.toISOString().slice(0,10),data_pagamento:null,valor:(base+(i===n?rest:0))/100,status:"pendente",metodo_pagamento:method,observacao:note?`${note} ${i}/${n}`:`Parcela ${i}/${n}`})}
   const {error}=await sb.from("recibo_pagamentos").insert(rows);if(error)return toast(error.message);await gerenciarPagamentosRecibo(reciboId);toast("Parcelas geradas e salvas com sucesso.");
 };
 enhanceAllSelects($("modal-body"));
}
async function removeReciboPagamento(paymentId,reciboId){
 if(!confirm("Deseja remover este pagamento/parcela?"))return;
 const {error}=await sb.from("recibo_pagamentos").delete().eq("id",paymentId);if(error)return toast(error.message);
 await atualizarStatusReciboFinanceiro(reciboId);await gerenciarPagamentosRecibo(reciboId);toast("Pagamento/parcela removido.");
}
...
function clientePDFHTML(c, titulo="Dados cadastrais do cliente"){
 if(!c)return `<div class="client-data"><div class="client-data-title">${esc(titulo)}</div><p>Cliente não localizado.</p></div>`;
 const endereco=[c.endereco,c.numero,c.complemento,c.bairro,c.cidade,c.estado].filter(Boolean).map(esc).join(" — ");
 const contatos=Array.isArray(c.contatos_json)?c.contatos_json:[];
 const campos=[
  ["Tipo de pessoa",c.tipo_pessoa],[c.tipo_pessoa==="PJ"?"Razão social / Nome":"Nome",c.nome],[c.tipo_pessoa==="PJ"?"CNPJ":"CPF",c.documento],
  ["Telefone",c.telefone],["Celular",c.celular],["E-mail",c.email],["E-mail de cobrança",c.email_cobranca],["CEP",c.cep],["Endereço completo",endereco]
 ].filter(x=>x[1]);
 let html=`<div class="client-data"><div class="client-data-title">${esc(titulo)}</div><div class="client-data-grid">${campos.map(([k,v])=>`<p><b>${esc(k)}:</b> ${v}</p>`).join("")}</div>`;
 if(c.tipo_pessoa==="PJ"){
  const rep=[["Representante legal",c.representante_legal_nome],["CPF do representante",c.representante_legal_cpf],["Cargo / função",c.representante_legal_cargo],["E-mail do representante",c.representante_legal_email],["Telefone do representante",c.representante_legal_telefone]].filter(x=>x[1]);
  if(rep.length)html+=`<div class="client-subtitle">Representante legal</div><div class="client-data-grid">${rep.map(([k,v])=>`<p><b>${esc(k)}:</b> ${esc(v)}</p>`).join("")}</div>`;
  if(contatos.length)html+=`<div class="client-subtitle">Contatos da empresa</div><div class="client-data-grid">${contatos.map(x=>`<p><b>${esc(x.nome||"Contato")}:</b> ${[x.telefone,x.email].filter(Boolean).map(esc).join(" • ")||"-"}</p>`).join("")}</div>`;
 }
 if(c.observacoes)html+=`<div class="client-subtitle">Observações cadastrais</div><p>${esc(c.observacoes)}</p>`;
 return html+`</div>`;
}
function clienteDoDocumento(doc){return clientes.find(c=>c.id===doc?.cliente_id)||doc?.clientes||null}
 ...
 *{box-sizing:border-box}
 ...
 .client-data{margin:12px 0 16px;padding:12px 14px;border:1px solid #e4e9f2;border-radius:9px;background:#fff}.client-data-title,.client-subtitle{font-size:10px;text-transform:uppercase;letter-spacing:.7px;color:#475467;font-weight:700;border-bottom:1px solid #e4e9f2;padding-bottom:5px;margin-bottom:7px}.client-subtitle{margin-top:10px}.client-data-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));column-gap:18px}.client-data p{margin:3px 0;overflow-wrap:anywhere}.client-data-grid p:has(b:first-child:nth-last-child(1)){min-width:0}
...
async function printOrcamento(id){
 ...
 printDocument(`Orçamento ${o.numero}`,`${clientePDFHTML(clienteDoDocumento(o))}<p><b>Data:</b> ${new Date(o.data_orcamento+"T12:00:00").toLocaleDateString("pt-BR")}</p><p><b>Validade:</b> ${o.validade_dias} dias</p>`,data||[],`<p class="total">Total: ${money(o.total)}</p><p class="muted">${esc(o.observacoes||"")}</p>`);
}
async function printOS(id){
 ...
 `${clientePDFHTML(clienteDoDocumento(o))}<p><b>Status:</b> ${esc(statusLabel(o.status))}</p>
...
function openModal(title,body){$("modal-title").textContent=title;$("modal-body").innerHTML=body;$("modal").classList.remove("hidden");setTimeout(()=>enhanceObservationEditors($("modal-body")),0)}
function sanitizeRichText(html){
 const t=document.createElement("template");t.innerHTML=String(html||"");
 // execCommand(foreColor) pode gerar <font color=...>. Convertemos antes de sanitizar
 // para preservar vermelho/preto no banco, na tela e nos PDFs.
 [...t.content.querySelectorAll("font")].forEach(el=>{const sp=document.createElement("span");const c=String(el.getAttribute("color")||"").toLowerCase();if(c==="red"||c==="#ff0000"||c==="#f00"||c==="rgb(255, 0, 0)")sp.style.color="#ff0000";else if(c==="black"||c==="#000000"||c==="#000"||c==="rgb(0, 0, 0)")sp.style.color="#000000";while(el.firstChild)sp.appendChild(el.firstChild);el.replaceWith(sp)});
 const ok=new Set(["B","STRONG","I","EM","U","BR","DIV","P","SPAN"]);
 [...t.content.querySelectorAll("*")].forEach(el=>{if(!ok.has(el.tagName)){el.replaceWith(...el.childNodes);return}[...el.attributes].forEach(a=>{if(el.tagName==="SPAN"&&a.name==="style"){const v=a.value.toLowerCase();if(/color\s*:\s*(red|#ff0000|#f00|rgb\(255,\s*0,\s*0\))/i.test(v))el.setAttribute("style","color:#ff0000");else if(/color\s*:\s*(black|#000000|#000|rgb\(0,\s*0,\s*0\))/i.test(v))el.setAttribute("style","color:#000000");else el.removeAttribute(a.name)}else el.removeAttribute(a.name)})});return t.innerHTML
}
function richDisplay(v){const x=String(v||"");return /<(b|strong|i|em|u|br|div|p|span)\b/i.test(x)?sanitizeRichText(x):esc(x).replaceAll("\n","<br>")}
function enhanceObservationEditors(root=document){root.querySelectorAll("textarea").forEach(ta=>{if(ta.dataset.richDone)return;const label=ta.closest("label");const hint=(label?.textContent||"")+" "+(ta.placeholder||"");if(!/observ|referente|descri[cç][aã]o|solicita[cç][aã]o/i.test(hint))return;ta.dataset.richDone="1";ta.classList.add("rich-source");const wrap=document.createElement("div");wrap.className="rich-editor-wrap";const bar=document.createElement("div");bar.className="rich-toolbar";bar.innerHTML='<button type="button" data-cmd="bold" title="Negrito (Ctrl+N)"><b>N</b></button><button type="button" data-cmd="italic" title="Itálico (Ctrl+I)"><i>I</i></button><button type="button" data-cmd="underline" title="Sublinhado (Ctrl+S)"><u>S</u></button><button type="button" data-cmd="toggleRed" title="Alternar texto vermelho/preto"><span class="red-a">A</span></button>';const ed=document.createElement("div");ed.className="rich-editor";ed.contentEditable="true";ed.innerHTML=richDisplay(ta.value);ta.parentNode.insertBefore(wrap,ta);wrap.append(bar,ed,ta);const sync=()=>ta.value=sanitizeRichText(ed.innerHTML);const selectionIsRed=()=>{const sel=window.getSelection();if(!sel||!sel.rangeCount)return false;let n=sel.anchorNode;n=n?.nodeType===3?n.parentElement:n;while(n&&n!==ed){if(n.nodeType===1){const c=(n.style?.color||getComputedStyle(n).color||'').toLowerCase();if(c==='red'||c==='#ff0000'||c==='rgb(255, 0, 0)')return true}n=n.parentElement}return false};bar.querySelectorAll("button").forEach(b=>b.onclick=()=>{ed.focus();if(b.dataset.cmd==="toggleRed"){document.execCommand("foreColor",false,selectionIsRed()?"#000000":"#ff0000")}else document.execCommand(b.dataset.cmd,false,b.dataset.value||null);sync()});ed.addEventListener("input",sync);ed.addEventListener("keydown",e=>{if(!e.ctrlKey)return;const k=e.key.toLowerCase();const cmd=k==="n"?"bold":k==="i"?"italic":k==="s"?"underline":null;if(cmd){e.preventDefault();document.execCommand(cmd,false,null);sync()}})})}

function closeModal(){$("modal").classList.add("hidden");editorItens=[];editorEditingIndex=-1;lmEditingIndex=-1}
...
function renderEditorItens(){
 ...
 tb.innerHTML=editorItens.map((i,n)=>editorEditingIndex===n?`<tr class="editor-edit-row"><td>${i.tipo==="material"?"Material":"Serviço"}</td><td><input id="ei-desc-${n}" value="${esc(i.descricao)}"></td><td><input id="ei-qtd-${n}" type="number" min="0.001" step="0.001" value="${i.quantidade}"></td><td><input id="ei-val-${n}" type="number" min="0" step="0.01" value="${i.valor_unitario}"></td><td>${money(i.quantidade*i.valor_unitario)}</td><td><div class="actions"><button type="button" class="action-btn" onclick="saveEditorItemEdit(${n})">Aplicar</button><button type="button" class="action-btn" onclick="editorEditingIndex=-1;renderEditorItens()">Cancelar</button></div></td></tr>`:`<tr><td>${i.tipo==="material"?"Material":"Serviço"}</td><td>${esc(i.descricao)}</td><td>${i.quantidade}</td><td>${money(i.valor_unitario)}</td><td>${money(i.quantidade*i.valor_unitario)}</td><td><div class="actions"><button type="button" class="action-btn" onclick="editEditorItem(${n})">Editar</button><button type="button" class="action-btn danger" onclick="removeEditorItem(${n})">Excluir</button></div></td></tr>`).join("")||`<tr><td colspan="6">Nenhum item adicionado.</td></tr>`;
...
function editEditorItem(n){editorEditingIndex=n;renderEditorItens()}
function saveEditorItemEdit(n){const i=editorItens[n];if(!i)return;const q=Number($("ei-qtd-"+n)?.value),v=Number($("ei-val-"+n)?.value);if(!Number.isFinite(q)||q<=0||!Number.isFinite(v)||v<0)return toast("Informe quantidade e valor válidos.");i.descricao=$("ei-desc-"+n)?.value.trim()||i.descricao;i.quantidade=q;i.valor_unitario=v;editorEditingIndex=-1;renderEditorItens()}
...
$("novo-cliente").onclick=()=>clienteForm();$("novo-material").onclick=()=>materialForm();$("nova-lista-material").onclick=()=>openListaMaterialForm();$("novo-servico").onclick=()=>servicoForm();$("nova-movimentacao").onclick=movementForm;
...
function listaMaterialForm(lista=null,itens=[]){
 ...
 <label>Vínculo<select id="lm-tipo" data-no-search="true"><option value="sem_vinculo" ${tipo==="sem_vinculo"?"selected":""}>Sem vínculo</option><option value="orcamento" ${tipo==="orcamento"?"selected":""}>Orçamento</option><option value="recibo" ${tipo==="recibo"?"selected":""}>Recibo</option><option value="ordem_servico" ${tipo==="ordem_servico"?"selected":""}>Ordem de Serviço (OS)</option></select></label>
 <label>Responsável pela emissão<input id="lm-responsavel" value="${esc(lista?.responsavel_emissao||currentUserProfile?.nome||currentSession?.user?.email||"")}" placeholder="Nome do responsável"></label><label>Cliente<select id="lm-cliente" data-no-search="true"><option value="">Selecione...</option>${clientes.map(c=>`<option value="${c.id}" ${c.id===lista?.cliente_id?"selected":""}>${esc(c.nome)}</option>`).join("")}</select></label>
 ...
 <label id="lm-rec-wrap" class="${tipo!=="recibo"?"hidden":""}">Recibo<select id="lm-rec"><option value="">Selecione...</option>${recibos.map(r=>`<option value="${r.id}" ${r.id===lista?.recibo_id?"selected":""}>${esc(r.numero)} — ${esc(r.clientes?.nome||"")}</option>`).join("")}</select></label><label id="lm-os-wrap" class="${tipo!=="ordem_servico"?"hidden":""}">Ordem de Serviço<select id="lm-os"><option value="">Selecione...</option>${ordens.map(o=>`<option value="${o.id}" ${o.id===lista?.ordem_servico_id?"selected":""}>${esc(o.numero)} — ${esc(o.clientes?.nome||"")}</option>`).join("")}</select></label>
 ...
 <label class="span-2">Material cadastrado<select id="lm-material" data-no-search="true"><option value="">Selecione...</option>${materiais.map(m=>`<option value="${m.id}">${esc(m.codigo||"")} — ${esc(m.nome)} — ${money(m.custo)}</option>`).join("")}</select></label>
 ...
 const syncVinculo=()=>{const t=$("lm-tipo").value;$("lm-orc-wrap").classList.toggle("hidden",t!=="orcamento");$("lm-rec-wrap").classList.toggle("hidden",t!=="recibo");$("lm-os-wrap").classList.toggle("hidden",t!=="ordem_servico")};
 ...
 $("lm-rec").onchange=()=>{const r=recibos.find(x=>x.id===$("lm-rec").value);if(r?.cliente_id)$("lm-cliente").value=r.cliente_id};$("lm-os").onchange=()=>{const o=ordens.find(x=>x.id===$("lm-os").value);if(o?.cliente_id)$("lm-cliente").value=o.cliente_id};
...
function enhanceSearchableSelect(select){
 if(!select || select.dataset?.noSearch==="true") return;
...
/* V041 — editor unifilar CAD-like, barramentos e biblioteca de símbolos */
const UF={
 entrada:{label:"Entrada",svg:'<path d="M30 4V44M20 14l10-10 10 10"/>'},medidor:{label:"Medidor",svg:'<path d="M2 26H12M48 26H58"/><rect x="12" y="13" width="36" height="26"/><line x1="21" y1="13" x2="21" y2="39"/><text x="35" y="30">kW.h</text>'},
 transformador:{label:"Transformador",svg:'<circle cx="24" cy="26" r="11"/><circle cx="38" cy="26" r="11"/>'},gerador:{label:"Gerador",svg:'<circle cx="30" cy="26" r="17"/><text x="30" y="31">G</text>'},
 quadro:{label:"Quadro",svg:'<rect x="12" y="6" width="36" height="40" stroke-dasharray="4 3"/><line x1="22" y1="12" x2="22" y2="40"/><line x1="30" y1="12" x2="30" y2="40"/><line x1="38" y1="12" x2="38" y2="40"/>'},
 barramento:{label:"Barramento",svg:'<rect x="5" y="20" width="50" height="12"/><circle cx="14" cy="26" r="2"/><circle cx="26" cy="26" r="2"/><circle cx="38" cy="26" r="2"/><circle cx="50" cy="26" r="2"/>'},
 pente:{label:"Barramento Pente",svg:'<path d="M5 34H55M12 34V15M24 34V15M36 34V15M48 34V15"/>'},barramento_n:{label:"Barramento N",svg:'<rect x="5" y="20" width="50" height="12"/><text x="30" y="29">N</text>'},
 disjuntor:{label:"Disjuntor",svg:'<path d="M30 2V13M30 39V50"/><circle cx="30" cy="13" r="2.5"/><circle cx="30" cy="39" r="2.5"/><path d="M30 16 C44 17 45 35 32 37"/>'},
 idr:{label:"IDR",svg:'<path d="M30 1V8M23 1V8M37 1V8M16 26H7M53 26H44"/><rect x="16" y="8" width="28" height="36"/><text x="30" y="31" style="font-size:18px">DR</text>'},dps:{label:"DPS",svg:'<path d="M30 2V8M30 44V50"/><rect x="20" y="8" width="20" height="36"/><text x="30" y="31" transform="rotate(-90 30 26)" style="font-size:8px">DPS CL II</text>'},
 fusivel:{label:"Fusível",svg:'<path d="M30 4V14M30 38V48"/><rect x="23" y="14" width="14" height="24"/>'},seccionadora:{label:"Seccionadora",svg:'<path d="M30 4V18M30 38V48M30 18L43 32"/><circle cx="30" cy="18" r="2"/><circle cx="30" cy="38" r="2"/>'},
 circuito:{label:"Circuito",svg:'<path d="M30 4V40"/><path d="M24 34L30 42L36 34"/>'},tomada:{label:"Tomada",svg:'<circle cx="30" cy="26" r="17"/><circle cx="24" cy="24" r="2"/><circle cx="36" cy="24" r="2"/><path d="M30 28V37"/>'},
 iluminacao:{label:"Iluminação",svg:'<circle cx="30" cy="26" r="17"/><path d="M18 14L42 38M42 14L18 38"/>'},motor:{label:"Motor",svg:'<circle cx="30" cy="26" r="17"/><text x="30" y="31">M</text>'},
 terra:{label:"PE / Terra",svg:'<path d="M30 4V25M16 25H44M20 31H40M24 37H36"/>'}
};
let ufCustomSymbols=[];
function ufId(){return crypto.randomUUID?.()||Math.random().toString(36).slice(2)}
function ufSvg(def){return `<svg viewBox="0 0 60 52" class="uf-svg" aria-hidden="true">${def||''}</svg>`}
function ufDef(n){if(n.type==="custom")return {label:n.custom_name||"Personalizado",svg:n.custom_svg||""};return UF[n.type]||{label:n.type,svg:'<rect x="12" y="10" width="36" height="32"/>'}}
async function loadUfCustomSymbols(){if(!currentUserProfile?.empresa_id)return;const {data,error}=await sb.from("unifilar_simbolos").select("*").eq("empresa_id",currentUserProfile.empresa_id).eq("ativo",true).order("nome");if(!error)ufCustomSymbols=data||[];renderUfCustomList()}
function renderUfCustomList(){const el=$("uf-custom-list");if(!el)return;el.innerHTML=ufCustomSymbols.map(s=>`<button type="button" data-uf-custom="${s.id}">${ufSvg(s.svg)}<span>${esc(s.nome)}</span></button>`).join("");el.querySelectorAll("[data-uf-custom]").forEach(b=>b.onclick=e=>{e.stopPropagation();selectUfInsertTool("custom",b.dataset.ufCustom)})}
function renderUnifilar(){if(!$("unifilar-canvas"))return;$("unifilar-name").value=unifilarDoc.nome||"";$("unifilar-vinculo-tipo").value=unifilarDoc.vinculo_tipo||"";unifilarDoc.sistema=unifilarDoc.sistema||"tri";$("unifilar-system").value=unifilarDoc.sistema;document.querySelectorAll("[data-uf-system]").forEach(b=>b.classList.toggle("active",b.dataset.ufSystem===unifilarDoc.sistema));updateUnifilarLinks();drawUnifilar();loadUfCustomSymbols()}
...
function ufDefaultSize(type){if(["barramento","pente","barramento_n"].includes(type))return {w:180,h:72};if(type==="quadro")return {w:150,h:110};return {w:96,h:96}}
function addUfNode(type,x,y,custom=null){const d=custom?{label:custom.nome,svg:custom.svg}:UF[type];if(!d)return;const sz=ufDefaultSize(custom?"custom":type);unifilarDoc.elementos.push({id:ufId(),type:custom?"custom":type,custom_symbol_id:custom?.id||null,custom_name:custom?.nome||null,custom_svg:custom?.svg||null,x:Math.max(10,Math.round(x/10)*10),y:Math.max(10,Math.round(y/10)*10),w:sz.w,h:sz.h,label:d.label,corrente:["idr","disjuntor"].includes(type)?40:null,sensibilidade:type==="idr"?30:null,polos:type==="idr"?2:null,potencia:type==="circuito"?1000:null,tensao:type==="circuito"?220:null,fp:1,material_id:null});unifilarInsertTool=null;updateUfToolSelection();drawUnifilar()}
function selectUfInsertTool(type,customId=null){unifilarInsertTool={type,customId};unifilarSelected=null;updateUfToolSelection();drawUfProperties();const c=$("unifilar-canvas");if(c)c.style.cursor="crosshair"}
function updateUfToolSelection(){document.querySelectorAll("[data-uf-type],[data-uf-custom]").forEach(b=>{const hit=unifilarInsertTool&&(b.dataset.ufType===unifilarInsertTool.type||b.dataset.ufCustom===unifilarInsertTool.customId);b.classList.toggle("uf-tool-active",!!hit)});const c=$("unifilar-canvas");if(c&&!unifilarInsertTool)c.style.cursor="default"}
function ufSub(n){if(n.type==="idr")return `${n.polos||2}P · ${n.corrente||40} A · IΔn ${n.sensibilidade||30} mA`;if(n.type==="disjuntor")return `${n.polos||1}P · ${n.corrente||20} A · ${n.icc||6} kA`;if(n.type==="circuito")return `${n.potencia||0} W · ${n.tensao||220} V`;if(n.type==="pente")return `${n.corrente||63} A`;return ""}
function drawUnifilar(){const box=$("unifilar-nodes"),svg=$("unifilar-lines");if(!box||!svg)return;box.innerHTML=unifilarDoc.elementos.map(n=>{const d=ufDef(n),sz=ufDefaultSize(n.type);n.w=Number(n.w)||sz.w;n.h=Number(n.h)||sz.h;return `<div class="uf-node uf-${n.type} ${unifilarSelected===n.id?"selected":""} ${unifilarConnectFrom===n.id?"connect-source":""}" data-uf-id="${n.id}" style="left:${n.x}px;top:${n.y}px;width:${n.w}px;height:${n.h}px;transform:rotate(${Number(n.rotation)||0}deg)"><i class="uf-port in" data-port="in"></i><div class="uf-symbol">${ufSvg(d.svg)}</div><div class="uf-label">${esc(n.label||d.label)}</div><div class="uf-sub">${esc(ufSub(n))}</div><i class="uf-port out" data-port="out"></i><i class="uf-resize" title="Redimensionar"></i></div>`}).join("");drawUfLines();box.querySelectorAll(".uf-node").forEach(el=>{el.onclick=e=>{e.stopPropagation();const id=el.dataset.ufId;if(e.target.closest(".uf-resize"))return;if(e.target.closest(".uf-port")?.dataset.port==="out"){unifilarConnectFrom=id;toast("Selecione a entrada do componente de destino");return}if(e.target.closest(".uf-port")?.dataset.port==="in"&&unifilarConnectFrom&&unifilarConnectFrom!==id){if(!unifilarDoc.conexoes.some(c=>c.from===unifilarConnectFrom&&c.to===id))unifilarDoc.conexoes.push({id:ufId(),from:unifilarConnectFrom,to:id,condutor:"fase"});unifilarConnectFrom=null;drawUnifilar();return}unifilarSelected=id;drawUnifilar()};el.onpointerdown=e=>{const n=unifilarDoc.elementos.find(x=>x.id===el.dataset.ufId),canvas=$("unifilar-canvas"),r=canvas.getBoundingClientRect();if(e.target.closest(".uf-port"))return;if(e.target.closest(".uf-resize")){e.preventDefault();e.stopPropagation();const sx=e.clientX,sy=e.clientY,sw=n.w,sh=n.h;el.setPointerCapture(e.pointerId);el.onpointermove=ev=>{n.w=Math.max(50,Math.round((sw+ev.clientX-sx)/10)*10);n.h=Math.max(50,Math.round((sh+ev.clientY-sy)/10)*10);el.style.width=n.w+"px";el.style.height=n.h+"px";drawUfLines()};el.onpointerup=()=>{el.onpointermove=null;drawUfProperties()};return}const dx=e.clientX-r.left+canvas.scrollLeft-n.x,dy=e.clientY-r.top+canvas.scrollTop-n.y;el.setPointerCapture(e.pointerId);el.onpointermove=ev=>{n.x=Math.max(0,Math.round((ev.clientX-r.left+canvas.scrollLeft-dx)/10)*10);n.y=Math.max(0,Math.round((ev.clientY-r.top+canvas.scrollTop-dy)/10)*10);el.style.left=n.x+"px";el.style.top=n.y+"px";drawUfLines()};el.onpointerup=()=>el.onpointermove=null}});drawUfProperties();calcUfSummary();updateUfToolSelection()}
function drawUfLines(){const by=Object.fromEntries(unifilarDoc.elementos.map(n=>[n.id,n]));$("unifilar-lines").innerHTML=unifilarDoc.conexoes.map(c=>{const a=by[c.from],b=by[c.to];if(!a||!b)return"";const x1=a.x+(Number(a.w)||96)/2,y1=a.y+(Number(a.h)||96),x2=b.x+(Number(b.w)||96)/2,y2=b.y,ym=Math.round((y1+y2)/2/10)*10;const color=c.condutor==="neutro"?"#13b5ea":c.condutor==="terra"?"#36d657":c.condutor==="fase"?"#ef2b2b":c.condutor==="fase_s"?"#d7dee7":c.condutor==="fase_t"?"#168cff":"#d7dee7";return `<path d="M ${x1} ${y1} V ${ym} H ${x2} V ${y2}" fill="none" stroke="${color}" stroke-width="2"/><circle cx="${x1}" cy="${ym}" r="3" fill="${color}"/>`}).join("")}
function drawUfProperties(){const p=$("unifilar-properties"),n=unifilarDoc.elementos.find(x=>x.id===unifilarSelected);if(!p)return;if(!n){p.innerHTML='<div class="uf-side-title">Propriedades</div><p class="muted">Selecione um componente.</p>';return}p.innerHTML=`<div class="uf-side-title">Propriedades</div><label>Identificação<input id="uf-label" value="${esc(n.label||"")}"></label>${["disjuntor","idr","pente","barramento"].includes(n.type)?`<label>Corrente (A)<input id="uf-current" type="number" value="${n.corrente||0}"></label>`:""}${["disjuntor","idr"].includes(n.type)?`<label>Polos<select id="uf-poles"><option>1</option><option>2</option><option>3</option><option>4</option></select></label>`:""}${n.type==="disjuntor"?`<label>Icc/Icn (kA)<input id="uf-icc" type="number" step="0.1" value="${n.icc||6}"></label>`:""}${n.type==="idr"?`<label>Sensibilidade IΔn (mA)<input id="uf-sens" type="number" value="${n.sensibilidade||30}"></label>`:""}${n.type==="circuito"?`<label>Potência (W)<input id="uf-power" type="number" value="${n.potencia||0}"></label><label>Tensão (V)<input id="uf-voltage" type="number" value="${n.tensao||220}"></label><label>FP<input id="uf-fp" type="number" step="0.01" value="${n.fp||1}"></label>`:""}<div class="uf-size-grid"><label>Largura<input id="uf-width" type="number" min="50" step="10" value="${n.w||96}"></label><label>Altura<input id="uf-height" type="number" min="50" step="10" value="${n.h||96}"></label></div><label>Material associado<select id="uf-material"><option value="">Nenhum</option>${materiais.map(m=>`<option value="${m.id}" ${m.id===n.material_id?"selected":""}>${esc((m.codigo||"")+" - "+m.nome)}</option>`).join("")}</select></label><button id="uf-delete" class="btn secondary full">Excluir componente</button>`;if($("uf-poles"))$("uf-poles").value=String(n.polos||1);p.querySelectorAll("input,select").forEach(x=>x.onchange=()=>{n.label=$("uf-label")?.value||n.label;if($("uf-current"))n.corrente=Number($("uf-current").value);if($("uf-poles"))n.polos=Number($("uf-poles").value);if($("uf-icc"))n.icc=Number($("uf-icc").value);if($("uf-sens"))n.sensibilidade=Number($("uf-sens").value);if($("uf-power"))n.potencia=Number($("uf-power").value);if($("uf-voltage"))n.tensao=Number($("uf-voltage").value);if($("uf-fp"))n.fp=Number($("uf-fp").value)||1;if($("uf-width"))n.w=Math.max(50,Number($("uf-width").value)||96);if($("uf-height"))n.h=Math.max(50,Number($("uf-height").value)||96);n.material_id=$("uf-material")?.value||null;drawUnifilar()});$("uf-delete").onclick=()=>{unifilarDoc.elementos=unifilarDoc.elementos.filter(x=>x.id!==n.id);unifilarDoc.conexoes=unifilarDoc.conexoes.filter(c=>c.from!==n.id&&c.to!==n.id);unifilarSelected=null;drawUnifilar()}}
...
async function loadUnifilar(id){const {data,error}=await sb.from("esquemas_unifilares").select("*").eq("id",id).single();if(error)return toast(error.message);unifilarDoc={...data,elementos:data.elementos||[],conexoes:data.conexoes||[],materiais:data.materiais||[],sistema:data.sistema||"tri"};unifilarSelected=null;closeModal();renderUnifilar()}
function newUnifilar(){unifilarDoc={id:null,nome:"Novo esquema",sistema:"tri",vinculo_tipo:"",vinculo_id:null,elementos:[],conexoes:[],materiais:[]};unifilarSelected=null;renderUnifilar()}
...
function showUnifilarMaterials(){const all=[...(unifilarDoc.materiais||[]),...unifilarDoc.elementos.filter(n=>n.material_id).map(n=>({material_id:n.material_id,quantidade:1}))];const map={};all.forEach(i=>{if(i.material_id)map[i.material_id]=(map[i.material_id]||0)+Number(i.quantidade||1)});const rows=Object.entries(map).map(([id,q])=>{const m=materiais.find(x=>x.id===id);return m?`<tr><td>${esc(m.codigo||"")}</td><td>${esc(m.nome)}</td><td>${Number(q).toLocaleString("pt-BR")}</td></tr>`:""}).join("");openModal("Materiais do esquema",`<div class="table-wrap"><table><thead><tr><th>Código</th><th>Material</th><th>Qtd.</th></tr></thead><tbody>${rows||'<tr><td colspan="3">Nenhum material associado ou importado.</td></tr>'}</tbody></table></div>`)}
function printUnifilar(){const c=$("unifilar-canvas").cloneNode(true);c.querySelectorAll('.uf-port').forEach(x=>x.remove());const w=window.open("","_blank");w.document.write(`<html><head><title>${esc(unifilarDoc.nome)}</title><style>body{font-family:Arial;background:#fff;color:#111;padding:12mm}.sheet{border:1px solid #111;padding:8mm}.canvas{position:relative;width:1100px;height:750px;overflow:hidden}.uf-node{position:absolute;width:120px;text-align:center}.uf-svg{width:60px;height:52px;fill:none;stroke:#111;stroke-width:1.7}.uf-svg text{fill:#111;stroke:none;text-anchor:middle;font:10px Arial}.uf-label{font-weight:700;font-size:11px}.uf-sub{font-size:9px}svg{position:absolute;width:2400px;height:1600px}.title{border-top:2px solid #111;margin-top:10px;padding-top:8px;display:flex;justify-content:space-between}</style></head><body><div class="sheet"><h2>DIAGRAMA UNIFILAR — ${esc(unifilarDoc.nome)}</h2><div class="canvas">${c.innerHTML}</div><div class="title"><b>${esc(empresa?.nome_fantasia||empresa?.razao_social||"Core Orça")}</b><span>${new Date().toLocaleDateString("pt-BR")} · S/E</span></div></div><script>onload=()=>print()<\/script></body></html>`);w.document.close()}
function openSymbolCreator(){openModal("Criar novo símbolo",`<div class="symbol-editor"><label>Nome<input id="ufs-name" placeholder="Ex.: Contator"></label><label>Categoria<input id="ufs-category" placeholder="Ex.: Proteção / Comando"></label><p class="muted">Desenhe com o mouse. O símbolo é salvo como geometria vetorial da sua empresa.</p><canvas id="ufs-canvas" width="360" height="240"></canvas><div class="action-group"><button id="ufs-clear" class="btn secondary">Limpar</button><button id="ufs-save" class="btn primary">Salvar símbolo</button></div></div>`);const cv=$("ufs-canvas"),ctx=cv.getContext("2d");let strokes=[],cur=null;function grid(){ctx.clearRect(0,0,cv.width,cv.height);ctx.strokeStyle="#dbe3ec";ctx.lineWidth=1;for(let x=0;x<cv.width;x+=20){ctx.beginPath();ctx.moveTo(x,0);ctx.lineTo(x,cv.height);ctx.stroke()}for(let y=0;y<cv.height;y+=20){ctx.beginPath();ctx.moveTo(0,y);ctx.lineTo(cv.width,y);ctx.stroke()}ctx.strokeStyle="#111827";ctx.lineWidth=2;strokes.forEach(s=>{ctx.beginPath();s.forEach((p,i)=>i?ctx.lineTo(p.x,p.y):ctx.moveTo(p.x,p.y));ctx.stroke()})}grid();cv.onpointerdown=e=>{cur=[];strokes.push(cur);cv.setPointerCapture(e.pointerId)};cv.onpointermove=e=>{if(!cur)return;const r=cv.getBoundingClientRect();cur.push({x:Math.round((e.clientX-r.left)/5)*5,y:Math.round((e.clientY-r.top)/5)*5});grid()};cv.onpointerup=()=>cur=null;$("ufs-clear").onclick=()=>{strokes=[];grid()};$("ufs-save").onclick=async()=>{const nome=$("ufs-name").value.trim();if(!nome||!strokes.length)return toast("Informe o nome e desenhe o símbolo.");const pts=strokes.flat();const minx=Math.min(...pts.map(p=>p.x)),maxx=Math.max(...pts.map(p=>p.x)),miny=Math.min(...pts.map(p=>p.y)),maxy=Math.max(...pts.map(p=>p.y)),w=Math.max(1,maxx-minx),h=Math.max(1,maxy-miny);const svg=strokes.map(s=>`<polyline points="${s.map(p=>`${((p.x-minx)/w*50+5).toFixed(1)},${((p.y-miny)/h*42+5).toFixed(1)}`).join(' ')}"/>`).join('');const {error}=await sb.from("unifilar_simbolos").insert({empresa_id:currentUserProfile.empresa_id,nome,categoria:$("ufs-category").value.trim()||"Personalizados",svg,ativo:true});if(error)return toast(error.message);closeModal();await loadUfCustomSymbols();toast("Símbolo criado")}}
document.addEventListener("DOMContentLoaded",()=>{document.querySelectorAll("[data-uf-type]").forEach(b=>{b.removeAttribute("draggable");b.onclick=e=>{e.stopPropagation();selectUfInsertTool(b.dataset.ufType)}});const c=$("unifilar-canvas");if(c){c.onclick=e=>{if(e.target.closest(".uf-node"))return;if(unifilarInsertTool){const r=c.getBoundingClientRect(),x=e.clientX-r.left+c.scrollLeft,y=e.clientY-r.top+c.scrollTop;if(unifilarInsertTool.customId){const cs=ufCustomSymbols.find(x=>x.id===unifilarInsertTool.customId);if(cs)addUfNode("custom",x,y,cs)}else addUfNode(unifilarInsertTool.type,x,y);return}unifilarSelected=null;drawUnifilar()}}$("unifilar-new")?.addEventListener("click",newUnifilar);$("unifilar-open")?.addEventListener("click",openUnifilarList);$("unifilar-organize")?.addEventListener("click",organizeUnifilar);$("unifilar-save")?.addEventListener("click",saveUnifilar);$("unifilar-pdf")?.addEventListener("click",printUnifilar);$("unifilar-materials")?.addEventListener("click",showUnifilarMaterials);$("unifilar-import-materials")?.addEventListener("click",importUnifilarLinkedMaterials);$("unifilar-vinculo-tipo")?.addEventListener("change",updateUnifilarLinks);$("unifilar-system")?.addEventListener("change",e=>{unifilarDoc.sistema=e.target.value;renderUnifilar()});document.querySelectorAll("[data-uf-system]").forEach(b=>b.onclick=()=>{$("unifilar-system").value=b.dataset.ufSystem;unifilarDoc.sistema=b.dataset.ufSystem;renderUnifilar()});$("uf-create-symbol")?.addEventListener("click",openSymbolCreator);$("uf-search")?.addEventListener("input",e=>{const q=e.target.value.toLowerCase();document.querySelectorAll(".unifilar-palette button[draggable=true]").forEach(b=>b.style.display=b.textContent.toLowerCase().includes(q)?"":"none")})});


/* V043 — ferramentas CAD e modo explícito de ligação */
let ufInteractionMode="select";
function setUfInteractionMode(mode){
  ufInteractionMode=mode;
  if(mode!=="connect") unifilarConnectFrom=null;
  document.body.classList.toggle("uf-connect-mode",mode==="connect");
  document.querySelectorAll("#uf-mode-select,#uf-mode-connect").forEach(b=>b?.classList.remove("active"));
  $(mode==="connect"?"uf-mode-connect":"uf-mode-select")?.classList.add("active");
  if($("uf-mode-status")) $("uf-mode-status").textContent=mode==="connect"?"Modo: Ligar — clique na origem e depois no destino":"Modo: Selecionar";
  drawUnifilar();
}
function deleteUfSelected(){if(!unifilarSelected)return;const id=unifilarSelected;unifilarDoc.elementos=unifilarDoc.elementos.filter(n=>n.id!==id);unifilarDoc.conexoes=unifilarDoc.conexoes.filter(c=>c.from!==id&&c.to!==id);unifilarSelected=null;drawUnifilar()}
function rotateUfSelected(){const n=unifilarDoc.elementos.find(n=>n.id===unifilarSelected);if(!n)return;n.rotation=((Number(n.rotation)||0)+90)%360;drawUnifilar()}
function ufConnectNodes(from,to){if(!from||!to||from===to)return;const condutor=$("uf-wire-type")?.value||"fase";if(!unifilarDoc.conexoes.some(c=>c.from===from&&c.to===to&&c.condutor===condutor))unifilarDoc.conexoes.push({id:ufId(),from,to,condutor});unifilarConnectFrom=null;drawUnifilar()}
document.addEventListener("click",e=>{
  if(ufInteractionMode!=="connect")return;
  const node=e.target.closest?.("#unifilar-nodes .uf-node");
  if(!node)return;
  e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();
  const id=node.dataset.ufId;
  if(!unifilarConnectFrom){unifilarConnectFrom=id;unifilarSelected=id;if($("uf-mode-status"))$("uf-mode-status").textContent="Origem selecionada — clique no componente de destino";drawUnifilar();return}
  ufConnectNodes(unifilarConnectFrom,id);
},true);
document.addEventListener("DOMContentLoaded",()=>{
  $("uf-mode-select")?.addEventListener("click",()=>setUfInteractionMode("select"));
  $("uf-mode-connect")?.addEventListener("click",()=>setUfInteractionMode("connect"));
  $("uf-delete-selected")?.addEventListener("click",deleteUfSelected);
  $("uf-rotate-selected")?.addEventListener("click",rotateUfSelected);
  document.addEventListener("keydown",e=>{if(new URLSearchParams(location.search).get("view")!=="unifilar")return;if(e.key==="Delete")deleteUfSelected();if(e.key==="Escape")setUfInteractionMode("select")});
});


/* =========================================================
   V045 — ESQUEMA VERTICAL
   Cadastro estruturado, edição completa, abertura com busca dinâmica,
   geração automática de pavimentos e PDFs profissionais.
   ========================================================= */
let obraEletricaAtual=null;
let obraEletricaPavimentos=[];
let obraEletricaEstruturas=[];
let obraEletricaComponentes=[];
let listaMaterialTopicos=[];

function oeDados(){return obraEletricaAtual?.dados||{}}
function oeOpt(){return obraEletricaAtual?.opcoes||{}}
function oeNum(v){return Number(v||0)}
function oeFmtKw(v){return v===null||v===undefined||v===''?'-':`${Number(v).toLocaleString('pt-BR')} kW`}
function oeTipoLabel(t){return ({apartamento:'Apartamento',quadro_medicao:'Quadro de medição',qgbt:'QGBT',subestacao:'Subestação',quadro_condominio:'Quadro de condomínio',lazer:'Lazer',garagem:'Garagem',gerador:'Gerador',barramento_blindado:'Barramento blindado',quadro:'Quadro',outro:'Outro'})[t]||String(t||'').replaceAll('_',' ')}
async function renderUnifilar(){
 if(!$('oe-empty'))return;
 if(!obraEletricaAtual){$('oe-empty').classList.remove('hidden');$('oe-work').classList.add('hidden');return}
 $('oe-empty').classList.add('hidden');$('oe-work').classList.remove('hidden');
 $('oe-title').textContent=obraEletricaAtual.nome||'-';$('oe-client').textContent=clientes.find(c=>c.id===obraEletricaAtual.cliente_id)?.nome||'-';$('oe-floor-count').textContent=obraEletricaPavimentos.length;$('oe-item-count').textContent=obraEletricaComponentes.length;
 const opts=oeOpt();document.querySelectorAll('[data-oe-option]').forEach(x=>x.checked=!!opts[x.dataset.oeOption]);renderObraEletricaPavimentos();
}
function oeValidUuid(v){return /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(String(v||'').trim())}
function oeEmpresaId(){const v=obraEletricaAtual?.empresa_id||currentUserProfile?.empresa_id||null;return oeValidUuid(v)?v:null}
function oeRequireUuid(v,label){const x=String(v??'').trim();if(!oeValidUuid(x)){toast(`${label} inválido. Reabra o esquema e tente novamente.`);return null}return x}
function oeMaterialOptions(sel=''){return '<option value="">Selecione o material...</option>'+materiais.map(m=>`<option value="${m.id}" ${m.id===sel?'selected':''}>${esc(m.codigo||'')} — ${esc(m.nome)}</option>`).join('')}
function oeOptionsHTML(o={}){return `<div class="oe-popup-section"><h4>O que existe na obra?</h4><div class="oe-choice-grid"><label><input id="oen-sub" type="checkbox" ${o.subestacao?'checked':''}> Subestação</label><label><input id="oen-lazer" type="checkbox" ${o.lazer?'checked':''}> Lazer</label><label><input id="oen-ger" type="checkbox" ${o.gerador?'checked':''}> Gerador</label><label><input id="oen-bus" type="checkbox" ${o.barramento_blindado?'checked':''}> Barramento blindado</label></div></div>`}
function oeTechnicalFieldsHTML(d={},o={}){return `<div class="oe-popup-section"><h4>Dados elétricos da obra</h4><div class="form-grid"><label>Carga instalada (kW)<input id="oen-carga" type="number" min="0" step="0.01" value="${d.carga_instalada??''}"></label><label>Demanda (kW)<input id="oen-demanda" type="number" min="0" step="0.01" value="${d.demanda??''}"></label><label>Quantidade de garagens<input id="oen-garagens" type="number" min="0" step="1" value="${d.qtd_garagens??0}"></label><label>Quantidade de pavimentos tipo<input id="oen-tipos" type="number" min="0" step="1" value="${d.qtd_tipos??0}"></label><label>Quantidade de quadros de medição<input id="oen-qm" type="number" min="0" step="1" value="${d.qtd_quadros_medicao??0}"></label><label>Quantidade total de pavimentos<input id="oen-total-pav" readonly value="${d.qtd_pavimentos??1}"></label></div><div id="oen-conditional"></div></div>`}
function refreshOeConditional(d={}){const box=$('oen-conditional');if(!box)return;let h='';if($('oen-bus')?.checked)h+=`<div class="form-grid oe-conditional"><label>Marca do barramento blindado<input id="oen-bus-marca" value="${esc(d.barramento_marca||'')}"></label><label>Modelo do barramento blindado<input id="oen-bus-modelo" value="${esc(d.barramento_modelo||'')}"></label></div>`;if($('oen-sub')?.checked)h+=`<div class="form-grid oe-conditional"><label>Quantidade de transformadores<input id="oen-trafos" type="number" min="1" step="1" value="${d.qtd_transformadores??1}"></label><label>Potências dos transformadores (kVA)<input id="oen-trafo-pot" value="${esc(d.transformadores_potencias||'')}" placeholder="Ex.: 500; 500; 300"></label></div>`;if($('oen-ger')?.checked)h+=`<div class="form-grid oe-conditional"><label>Marca do gerador<input id="oen-ger-marca" value="${esc(d.gerador_marca||'')}"></label><label>Potência do gerador (kVA)<input id="oen-ger-pot" type="number" min="0" step="0.01" value="${d.gerador_potencia??''}"></label></div>`;box.innerHTML=h;const upd=()=>{const g=oeNum($('oen-garagens')?.value),t=oeNum($('oen-tipos')?.value),l=$('oen-lazer')?.checked?1:0;if($('oen-total-pav'))$('oen-total-pav').value=1+g+l+t};['oen-garagens','oen-tipos','oen-lazer'].forEach(id=>$(id)?.addEventListener('input',upd));upd()}
function collectOeForm(){const opcoes={subestacao:$('oen-sub').checked,lazer:$('oen-lazer').checked,gerador:$('oen-ger').checked,barramento_blindado:$('oen-bus').checked};const dados={carga_instalada:$('oen-carga').value===''?null:oeNum($('oen-carga').value),demanda:$('oen-demanda').value===''?null:oeNum($('oen-demanda').value),qtd_garagens:oeNum($('oen-garagens').value),qtd_tipos:oeNum($('oen-tipos').value),qtd_quadros_medicao:oeNum($('oen-qm').value),qtd_pavimentos:oeNum($('oen-total-pav').value),barramento_marca:$('oen-bus-marca')?.value.trim()||null,barramento_modelo:$('oen-bus-modelo')?.value.trim()||null,qtd_transformadores:oeNum($('oen-trafos')?.value),transformadores_potencias:$('oen-trafo-pot')?.value.trim()||null,gerador_marca:$('oen-ger-marca')?.value.trim()||null,gerador_potencia:$('oen-ger-pot')?.value===''?null:oeNum($('oen-ger-pot')?.value)};return {opcoes,dados}}
function autoFloorNames(d,o){const names=['Térreo'];for(let i=1;i<=oeNum(d.qtd_garagens);i++)names.push(`G${i}`);if(o.lazer)names.push('Lazer');for(let i=1;i<=oeNum(d.qtd_tipos);i++)names.push(`Tipo ${String(i).padStart(2,'0')}`);return names}
async function syncAutoPavimentos(obraId,d,o){const wanted=autoFloorNames(d,o);const existing=obraEletricaPavimentos.length?obraEletricaPavimentos:(await sb.from('obra_eletrica_pavimentos').select('*').eq('obra_id',obraId).order('ordem')).data||[];for(let i=0;i<wanted.length;i++){if(existing.some(p=>p.nome.toLowerCase()===wanted[i].toLowerCase()))continue;const r=await sb.from('obra_eletrica_pavimentos').insert({obra_id:obraId,nome:wanted[i],ordem:i}).select().single();if(!r.error)existing.push(r.data)}obraEletricaPavimentos=existing.sort((a,b)=>a.ordem-b.ordem)}
async function syncQuadrosMedicao(){const qtd=oeNum(oeDados().qtd_quadros_medicao);if(!qtd||!obraEletricaPavimentos.length)return;const floor=obraEletricaPavimentos.find(p=>p.nome==='Térreo')||obraEletricaPavimentos[0];for(let i=1;i<=qtd;i++){const nome=`QM-${String(i).padStart(2,'0')}`;if(obraEletricaEstruturas.some(s=>s.tipo==='quadro_medicao'&&s.nome===nome))continue;const r=await sb.from('obra_eletrica_estruturas').insert({obra_id:obraEletricaAtual.id,pavimento_id:floor.id,tipo:'quadro_medicao',nome,sistema:'Trifásico',observacoes:null,detalhes:{gerado_automaticamente:true},ordem:obraEletricaEstruturas.length}).select().single();if(!r.error)obraEletricaEstruturas.push(r.data)}}
async function novaObraEletrica(){openModal('Novo esquema vertical',`<form id="oe-new-form"><div class="form-grid"><label>Nome da obra*<input id="oen-name" required></label><label>Cliente<select id="oen-client"><option value="">Sem cliente</option>${clientes.map(c=>`<option value="${c.id}">${esc(c.nome)}</option>`).join('')}</select></label><label class="span-2">Observações<textarea id="oen-obs"></textarea></label></div>${oeOptionsHTML()}${oeTechnicalFieldsHTML()}<div class="modal-actions"><button type="button" class="btn secondary" onclick="closeModal()">Cancelar</button><button class="btn primary">Criar esquema</button></div></form>`);['oen-sub','oen-lazer','oen-ger','oen-bus'].forEach(id=>$(id).onchange=()=>refreshOeConditional());refreshOeConditional();$('oe-new-form').onsubmit=async e=>{e.preventDefault();const empresaId=oeEmpresaId();if(!empresaId)return toast('Não foi possível identificar a empresa ativa. Saia e entre novamente.');const {opcoes,dados}=collectOeForm();const payload={nome:$('oen-name').value.trim(),cliente_id:$('oen-client').value||null,observacoes:$('oen-obs').value.trim()||null,opcoes,dados};const {data,error}=await sb.from('obras_eletricas').insert(payload).select().single();if(error)return toast(error.message);obraEletricaAtual=data;obraEletricaPavimentos=[];obraEletricaEstruturas=[];obraEletricaComponentes=[];await syncAutoPavimentos(data.id,dados,opcoes);await syncQuadrosMedicao();closeModal();renderUnifilar();toast('Esquema vertical criado') }}
async function editarObraEletrica(){if(!obraEletricaAtual)return toast('Abra um esquema vertical.');const d=oeDados(),o=oeOpt();openModal('Editar dados da obra',`<form id="oe-edit-form"><div class="form-grid"><label>Nome da obra*<input id="oen-name" required value="${esc(obraEletricaAtual.nome||'')}"></label><label>Cliente<select id="oen-client"><option value="">Sem cliente</option>${clientes.map(c=>`<option value="${c.id}" ${c.id===obraEletricaAtual.cliente_id?'selected':''}>${esc(c.nome)}</option>`).join('')}</select></label><label class="span-2">Observações<textarea id="oen-obs">${esc(obraEletricaAtual.observacoes||'')}</textarea></label></div>${oeOptionsHTML(o)}${oeTechnicalFieldsHTML(d,o)}<div class="modal-actions"><button type="button" class="btn secondary" onclick="closeModal()">Cancelar</button><button class="btn primary">Salvar alterações</button></div></form>`);['oen-sub','oen-lazer','oen-ger','oen-bus'].forEach(id=>$(id).onchange=()=>refreshOeConditional(d));refreshOeConditional(d);$('oe-edit-form').onsubmit=async e=>{e.preventDefault();const {opcoes,dados}=collectOeForm();const patch={nome:$('oen-name').value.trim(),cliente_id:$('oen-client').value||null,observacoes:$('oen-obs').value.trim()||null,opcoes,dados,updated_at:new Date().toISOString()};const r=await sb.from('obras_eletricas').update(patch).eq('id',obraEletricaAtual.id).select().single();if(r.error)return toast(r.error.message);obraEletricaAtual=r.data;await syncAutoPavimentos(r.data.id,dados,opcoes);await syncQuadrosMedicao();closeModal();renderUnifilar();toast('Dados atualizados')}}
async function abrirObraEletrica(){const {data,error}=await sb.from('obras_eletricas').select('*').order('updated_at',{ascending:false});if(error)return toast(error.message);const rows=data||[];openModal('Abrir esquema vertical',`<div class="oe-open-search"><input id="oe-open-search" placeholder="Pesquisar obra ou cliente..."></div><div id="oe-open-results" class="license-list"></div>`);const draw=()=>{const q=($('oe-open-search').value||'').toLowerCase().trim();const f=rows.filter(o=>{const cli=clientes.find(c=>c.id===o.cliente_id)?.nome||'';return !q||`${o.nome} ${cli}`.toLowerCase().includes(q)});$('oe-open-results').innerHTML=f.map(o=>{const cli=clientes.find(c=>c.id===o.cliente_id)?.nome||'Sem cliente';return `<div class="license-row"><span><b>${esc(o.nome)}</b><br><small>${esc(cli)} • Atualizado ${new Date(o.updated_at).toLocaleString('pt-BR')}</small></span><button class="action-btn" onclick="carregarObraEletrica('${o.id}')">Abrir</button></div>`}).join('')||'<p>Nenhuma obra encontrada.</p>'};$('oe-open-search').oninput=draw;draw()}
async function carregarObraEletrica(id){const [a,b,c,d]=await Promise.all([sb.from('obras_eletricas').select('*').eq('id',id).single(),sb.from('obra_eletrica_pavimentos').select('*').eq('obra_id',id).order('ordem'),sb.from('obra_eletrica_estruturas').select('*').eq('obra_id',id).order('ordem'),sb.from('obra_eletrica_componentes').select('*').eq('obra_id',id).order('ordem')]);if(a.error||b.error||c.error||d.error)return toast(a.error?.message||b.error?.message||c.error?.message||d.error?.message);obraEletricaAtual=a.data;obraEletricaPavimentos=b.data||[];obraEletricaEstruturas=c.data||[];obraEletricaComponentes=d.data||[];closeModal();renderUnifilar()}
async function salvarOpcoesObra(){if(!obraEletricaAtual)return;const opcoes={...oeOpt()};document.querySelectorAll('[data-oe-option]').forEach(x=>opcoes[x.dataset.oeOption]=x.checked);obraEletricaAtual.opcoes=opcoes;const {error}=await sb.from('obras_eletricas').update({opcoes,updated_at:new Date().toISOString()}).eq('id',obraEletricaAtual.id);if(error)return toast(error.message);toast('Esquema salvo')}
function adicionarPavimento(p=null){if(!obraEletricaAtual)return toast('Crie ou abra um esquema.');openModal(p?'Editar pavimento':'Adicionar pavimento',`<form id="oe-floor-form"><div class="form-grid"><label>Nome*<input id="oef-name" required value="${esc(p?.nome||'')}" placeholder="Ex.: Térreo, G1, Lazer, Tipo 01"></label><label>Ordem<input id="oef-order" type="number" value="${p?.ordem??obraEletricaPavimentos.length}"></label></div><div class="modal-actions"><button type="button" class="btn secondary" onclick="closeModal()">Cancelar</button><button class="btn primary">Salvar</button></div></form>`);$('oe-floor-form').onsubmit=async e=>{e.preventDefault();const payload={nome:$('oef-name').value.trim(),ordem:Number($('oef-order').value)||0};let r=p?await sb.from('obra_eletrica_pavimentos').update(payload).eq('id',p.id).select().single():await sb.from('obra_eletrica_pavimentos').insert({...payload,obra_id:obraEletricaAtual.id}).select().single();if(r.error)return toast(r.error.message);if(p)obraEletricaPavimentos=obraEletricaPavimentos.map(x=>x.id===p.id?r.data:x);else obraEletricaPavimentos.push(r.data);closeModal();renderUnifilar()}}
function editarPavimento(id){const p=obraEletricaPavimentos.find(x=>x.id===id);if(p)adicionarPavimento(p)}
function adicionarEstrutura(tipoInicial='quadro',s0=null){if(!obraEletricaAtual)return toast('Crie ou abra um esquema.');if(!obraEletricaPavimentos.length)return toast('Cadastre primeiro um pavimento.');const tipos=['apartamento','quadro_medicao','qgbt','subestacao','quadro_condominio','lazer','garagem','gerador','barramento_blindado','quadro','outro'];const det=s0?.detalhes||{};openModal(s0?'Editar quadro / equipamento':(tipoInicial==='apartamento'?'Adicionar apartamento':'Adicionar quadro / equipamento'),`<form id="oe-struct-form"><div class="form-grid"><label>Tipo<select id="oes-type">${tipos.map(t=>`<option value="${t}" ${t===(s0?.tipo||tipoInicial)?'selected':''}>${oeTipoLabel(t)}</option>`).join('')}</select></label><label>Identificação*<input id="oes-name" required value="${esc(s0?.nome||'')}" placeholder="Ex.: Apto 101, QD-101, QGBT"></label><label>Pavimento<select id="oes-floor">${obraEletricaPavimentos.map(p=>`<option value="${p.id}" ${p.id===s0?.pavimento_id?'selected':''}>${esc(p.nome)}</option>`).join('')}</select></label><label>Sistema<select id="oes-system">${['Monofásico','Bifásico','Trifásico'].map(x=>`<option ${x===s0?.sistema?'selected':''}>${x}</option>`).join('')}</select></label><label>Carga instalada (kW)<input id="oes-carga" type="number" min="0" step="0.01" value="${det.carga_instalada??''}"></label><label>Demanda (kW)<input id="oes-demanda" type="number" min="0" step="0.01" value="${det.demanda??''}"></label><label class="span-2">Observações<textarea id="oes-obs">${esc(s0?.observacoes||'')}</textarea></label></div><div class="modal-actions"><button type="button" class="btn secondary" onclick="closeModal()">Cancelar</button><button class="btn primary">Salvar</button></div></form>`);$('oe-struct-form').onsubmit=async e=>{e.preventDefault();const payload={pavimento_id:$('oes-floor').value,tipo:$('oes-type').value,nome:$('oes-name').value.trim(),sistema:$('oes-system').value,observacoes:$('oes-obs').value.trim()||null,detalhes:{...det,carga_instalada:$('oes-carga').value===''?null:oeNum($('oes-carga').value),demanda:$('oes-demanda').value===''?null:oeNum($('oes-demanda').value)}};let r=s0?await sb.from('obra_eletrica_estruturas').update(payload).eq('id',s0.id).select().single():await sb.from('obra_eletrica_estruturas').insert({...payload,obra_id:obraEletricaAtual.id,ordem:obraEletricaEstruturas.length}).select().single();if(r.error)return toast(r.error.message);if(s0){obraEletricaEstruturas=obraEletricaEstruturas.map(x=>x.id===s0.id?r.data:x);await sb.from('obra_eletrica_componentes').update({pavimento_id:r.data.pavimento_id}).eq('estrutura_id',r.data.id);obraEletricaComponentes=obraEletricaComponentes.map(c=>c.estrutura_id===r.data.id?{...c,pavimento_id:r.data.pavimento_id}:c)}else obraEletricaEstruturas.push(r.data);closeModal();renderUnifilar()}}
function editarEstrutura(id){const s=obraEletricaEstruturas.find(x=>x.id===id);if(s)adicionarEstrutura(s.tipo,s)}
function adicionarComponente(c0=null){
 if(!obraEletricaAtual)return toast('Crie ou abra um esquema vertical.');
 if(!obraEletricaEstruturas.length)return toast('Cadastre primeiro um apartamento, quadro ou equipamento.');
 const tipos=['cabo_entrada','cabo_saida','disjuntor','idr','dps','barramento_pente','barramento_neutro','barramento_terra','medidor','transformador','gerador','outro'];
 openModal(c0?'Editar componente':'Adicionar componente',`<form id="oe-comp-form"><div class="form-grid"><label>Local / quadro<select id="oec-struct" data-no-search="true">${obraEletricaEstruturas.map(s=>`<option value="${s.id}" ${s.id===c0?.estrutura_id?'selected':''}>${esc(s.nome)}</option>`).join('')}</select></label><label>Tipo<select id="oec-type" data-no-search="true">${tipos.map(t=>`<option value="${t}" ${t===c0?.tipo?'selected':''}>${t.replaceAll('_',' ')}</option>`).join('')}</select></label><label class="span-2">Material do banco*<select id="oec-material" required>${oeMaterialOptions(c0?.material_id)}</select></label><label>Quantidade<input id="oec-qtd" type="number" min="0.01" step="0.01" value="${c0?.quantidade??1}"></label><label>Identificação<input id="oec-label" value="${esc(c0?.identificacao||'')}" placeholder="Ex.: DJ Geral, IDR-01"></label><label class="span-2">Observações técnicas<input id="oec-obs" value="${esc(c0?.observacoes||'')}" placeholder="Marca, modelo, cor, corrente, seção, comprimento..."></label></div><div class="modal-actions"><button type="button" class="btn secondary" onclick="closeModal()">Cancelar</button><button class="btn primary">Salvar</button></div></form>`);
 enhanceSearchableSelect($('oec-material'));
 $('oe-comp-form').onsubmit=async e=>{
  e.preventDefault();
  const sid=oeRequireUuid($('oec-struct')?.value,'Local / quadro'); if(!sid)return;
  const mid=oeRequireUuid($('oec-material')?.value,'Material'); if(!mid)return;
  const st=obraEletricaEstruturas.find(x=>String(x.id)===sid);
  if(!st)return toast('O quadro selecionado não está carregado. Reabra o esquema.');
  if(!materiais.some(x=>String(x.id)===mid))return toast('O material selecionado não está disponível para esta empresa.');
  const payload={pavimento_id:st.pavimento_id,estrutura_id:st.id,tipo:$('oec-type').value,material_id:mid,quantidade:Number($('oec-qtd').value)||1,identificacao:$('oec-label').value.trim()||null,observacoes:$('oec-obs').value.trim()||null};
  if(!oeValidUuid(payload.pavimento_id))return toast('O quadro selecionado não possui pavimento válido. Edite o quadro e selecione o pavimento.');
  const empresaId=oeEmpresaId(); if(!empresaId)return toast('Empresa ativa inválida. Saia e entre novamente.');
  const obraId=oeRequireUuid(obraEletricaAtual?.id,'Obra'); if(!obraId)return;
  let r=c0?await sb.from('obra_eletrica_componentes').update(payload).eq('id',c0.id).select().single():await sb.from('obra_eletrica_componentes').insert({...payload,obra_id:obraId,ordem:obraEletricaComponentes.length}).select().single();
  if(r.error)return toast(r.error.message);
  if(c0)obraEletricaComponentes=obraEletricaComponentes.map(x=>x.id===c0.id?r.data:x);else obraEletricaComponentes.push(r.data);
  closeModal();renderUnifilar();toast(c0?'Componente atualizado.':'Componente adicionado.');
 };
}
function editarComponente(id){const c=obraEletricaComponentes.find(x=>x.id===id);if(c)adicionarComponente(c)}
async function excluirComponente(id){if(!confirm('Excluir este componente?'))return;const r=await sb.from('obra_eletrica_componentes').delete().eq('id',id);if(r.error)return toast(r.error.message);obraEletricaComponentes=obraEletricaComponentes.filter(x=>x.id!==id);renderUnifilar()}
async function excluirEstrutura(id){const st=obraEletricaEstruturas.find(x=>x.id===id);if(!st||!confirm(`Excluir ${st.nome} e todos os componentes vinculados?`))return;let r=await sb.from('obra_eletrica_componentes').delete().eq('estrutura_id',id);if(r.error)return toast(r.error.message);r=await sb.from('obra_eletrica_estruturas').delete().eq('id',id);if(r.error)return toast(r.error.message);obraEletricaComponentes=obraEletricaComponentes.filter(x=>x.estrutura_id!==id);obraEletricaEstruturas=obraEletricaEstruturas.filter(x=>x.id!==id);renderUnifilar();toast('Quadro / equipamento excluído.')}
async function excluirPavimento(id){const p=obraEletricaPavimentos.find(x=>x.id===id);if(!p||!confirm(`Excluir o pavimento ${p.nome}, seus quadros e componentes?`))return;const ids=obraEletricaEstruturas.filter(x=>x.pavimento_id===id).map(x=>x.id);for(const sid of ids){const a=await sb.from('obra_eletrica_componentes').delete().eq('estrutura_id',sid);if(a.error)return toast(a.error.message)}let r=await sb.from('obra_eletrica_estruturas').delete().eq('pavimento_id',id);if(r.error)return toast(r.error.message);r=await sb.from('obra_eletrica_pavimentos').delete().eq('id',id);if(r.error)return toast(r.error.message);obraEletricaComponentes=obraEletricaComponentes.filter(x=>x.pavimento_id!==id);obraEletricaEstruturas=obraEletricaEstruturas.filter(x=>x.pavimento_id!==id);obraEletricaPavimentos=obraEletricaPavimentos.filter(x=>x.id!==id);renderUnifilar();toast('Pavimento excluído.')}
async function excluirObraEletrica(){if(!obraEletricaAtual||!confirm(`Excluir definitivamente o esquema vertical ${obraEletricaAtual.nome} e todos os seus cadastros?`))return;const id=obraEletricaAtual.id;let r=await sb.from('obra_eletrica_componentes').delete().eq('obra_id',id);if(r.error)return toast(r.error.message);r=await sb.from('obra_eletrica_estruturas').delete().eq('obra_id',id);if(r.error)return toast(r.error.message);r=await sb.from('obra_eletrica_pavimentos').delete().eq('obra_id',id);if(r.error)return toast(r.error.message);r=await sb.from('obras_eletricas').delete().eq('id',id);if(r.error)return toast(r.error.message);obraEletricaAtual=null;obraEletricaPavimentos=[];obraEletricaEstruturas=[];obraEletricaComponentes=[];renderUnifilar();toast('Esquema vertical excluído.')}
function renderObraEletricaPavimentos(){const el=$('oe-floors');if(!el)return;el.innerHTML=obraEletricaPavimentos.map(p=>{const ss=obraEletricaEstruturas.filter(s=>s.pavimento_id===p.id);return `<section class="oe-floor"><header><div><small>PAVIMENTO</small><h3>${esc(p.nome)}</h3></div><div class="oe-card-actions"><span>${ss.length} cadastro(s)</span><button class="action-btn" onclick="editarPavimento('${p.id}')">Editar</button><button class="action-btn danger" onclick="excluirPavimento('${p.id}')">Excluir</button></div></header><div class="oe-struct-grid">${ss.map(s=>{const cc=obraEletricaComponentes.filter(c=>c.estrutura_id===s.id),det=s.detalhes||{};return `<article class="oe-struct"><div class="oe-struct-head"><div><small>${esc(oeTipoLabel(s.tipo).toUpperCase())}</small><b>${esc(s.nome)}</b><em>${esc(s.sistema||'')}${det.carga_instalada!=null?` • Carga ${oeFmtKw(det.carga_instalada)}`:''}${det.demanda!=null?` • Demanda ${oeFmtKw(det.demanda)}`:''}</em></div><div class="oe-inline-actions"><button class="action-btn" onclick="editarEstrutura('${s.id}')">Editar</button><button class="action-btn danger" onclick="excluirEstrutura('${s.id}')">Excluir</button></div></div><div class="oe-components">${cc.map(c=>{const m=materiais.find(x=>x.id===c.material_id);return `<div class="oe-component"><span><b>${esc(c.identificacao||c.tipo.replaceAll('_',' '))}</b><small>${esc(m?.nome||'Material não encontrado')}${c.observacoes?'<br>'+esc(c.observacoes):''}</small></span><strong>${Number(c.quantidade).toLocaleString('pt-BR')} ${esc(m?.unidade||'un')}</strong><div class="oe-inline-actions"><button class="action-btn" onclick="editarComponente('${c.id}')">Editar</button><button class="action-btn danger" onclick="excluirComponente('${c.id}')">Excluir</button></div></div>`}).join('')||'<span class="muted">Nenhum componente.</span>'}</div></article>`}).join('')||'<p class="muted">Nenhum quadro/equipamento neste pavimento.</p>'}</div></section>`}).join('')||'<div class="oe-empty"><b>Cadastre o primeiro pavimento.</b></div>'}
function materiaisDaObra(){const map={};obraEletricaComponentes.forEach(c=>{if(!c.material_id)return;const k=c.material_id;if(!map[k])map[k]={material_id:k,quantidade:0,observacoes:new Set()};map[k].quantidade+=Number(c.quantidade)||0;if(c.observacoes)map[k].observacoes.add(c.observacoes)});return Object.values(map).map(x=>({...x,observacoes:[...x.observacoes].join('; ')}))}
async function gerarListaMateriaisObra(){if(!obraEletricaAtual)return toast('Abra um esquema vertical.');const rows=materiaisDaObra();if(!rows.length)return toast('A obra ainda não possui materiais vinculados.');const {data:l,error}=await sb.from('listas_materiais').insert({nome:`Materiais — ${obraEletricaAtual.nome}`,data_lista:today(),cliente_id:obraEletricaAtual.cliente_id||null,responsavel_emissao:currentUserProfile?.nome||currentSession?.user?.email||'',tipo_vinculo:'sem_vinculo',observacoes:`Gerada automaticamente a partir do Esquema Vertical: ${obraEletricaAtual.nome}`,total:0,topicos:['Materiais do Esquema Vertical']}).select().single();if(error)return toast(error.message);const payload=rows.map((r,n)=>{const m=materiais.find(x=>x.id===r.material_id);const details=[m?.fabricante?`Marca: ${m.fabricante}`:'',r.observacoes||''].filter(Boolean).join('\n');return {lista_id:l.id,material_id:r.material_id,codigo:m?.codigo||null,descricao:m?.nome||'Material',categoria:m?.categoria||null,unidade:m?.unidade||'un',quantidade:r.quantidade,valor_unitario:0,observacoes:details||null,ordem:n,topico:'Materiais do Esquema Vertical'}});const ins=await sb.from('lista_materiais_itens').insert(payload);if(ins.error)return toast(ins.error.message);await loadListasMateriais();toast('Lista de materiais gerada sem valores.')}
function esquemaPavimentosHTML(){return obraEletricaPavimentos.map(p=>{const ss=obraEletricaEstruturas.filter(s=>s.pavimento_id===p.id);return `<section class="floor-block"><div class="floor-title"><span>PAVIMENTO</span><h2>${esc(p.nome)}</h2></div>${ss.map(s=>{const cc=obraEletricaComponentes.filter(c=>c.estrutura_id===s.id),det=s.detalhes||{};return `<div class="vertical-card"><div class="vertical-card-head"><div><small>${esc(oeTipoLabel(s.tipo).toUpperCase())}</small><h3>${esc(s.nome)}</h3></div><div class="vertical-meta">${esc(s.sistema||'')}${det.carga_instalada!=null?`<br>Carga: ${oeFmtKw(det.carga_instalada)}`:''}${det.demanda!=null?`<br>Demanda: ${oeFmtKw(det.demanda)}`:''}</div></div>${cc.map(c=>{const m=materiais.find(x=>x.id===c.material_id);return `<div class="vertical-line"><span class="vertical-dot"></span><div><b>${esc(c.identificacao||c.tipo.replaceAll('_',' '))}</b><small>${esc(m?.nome||'Material')}${c.observacoes?` • ${esc(c.observacoes)}`:''}</small></div><strong>${Number(c.quantidade).toLocaleString('pt-BR')} ${esc(m?.unidade||'un')}</strong></div>`}).join('')||'<p class="empty-line">Sem componentes cadastrados.</p>'}</div>`}).join('')||'<p class="empty-line">Sem quadros/equipamentos cadastrados.</p>'}</section>`}).join('')}
function printBrandHeader(title,subtitle=''){const e=empresa||{},logo=e.logo_url?`<img class="brand-logo" src="${esc(e.logo_url)}">`:`<div class="brand-mark">⚡</div>`,meta=[e.cnpj?`CNPJ ${esc(e.cnpj)}`:'',e.telefone?esc(e.telefone):'',e.email?esc(e.email):''].filter(Boolean).join(' • ');return `<div class="top"><div class="brand">${logo}<div class="company"><h1>${esc(e.nome_fantasia||e.razao_social||'Core Orça')}</h1><p>${meta}</p><p>${esc([e.endereco,e.cidade,e.uf].filter(Boolean).join(' — '))}</p></div></div><div class="doc-title"><div class="type">${esc(subtitle)}</div><h2>${esc(title)}</h2></div></div>`}
function imprimirEsquemaPavimentos(){if(!obraEletricaAtual)return toast('Abra um esquema vertical.');const d=oeDados(),o=oeOpt(),cli=clientes.find(c=>c.id===obraEletricaAtual.cliente_id),w=window.open('','_blank');if(!w)return toast('Permita pop-ups para gerar o PDF.');const extras=[o.subestacao?`Subestação: ${d.qtd_transformadores||'-'} trafo(s) • ${d.transformadores_potencias||'-'} kVA`:'',o.gerador?`Gerador: ${d.gerador_marca||'-'} • ${d.gerador_potencia||'-'} kVA`:'',o.barramento_blindado?`Barramento blindado: ${d.barramento_marca||'-'} ${d.barramento_modelo||''}`:''].filter(Boolean);w.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>${esc(obraEletricaAtual.nome)}</title><style>@page{size:A4 landscape;margin:11mm 12mm 13mm}*{box-sizing:border-box}body{font:10px Arial;color:#172033;margin:0}.doc{width:100%}.top{display:flex;justify-content:space-between;align-items:flex-start;padding-bottom:10px;border-bottom:2px solid #1f4fd6}.brand{display:flex;gap:10px;align-items:center}.brand-logo{max-width:140px;max-height:50px}.brand-mark{width:38px;height:38px;border-radius:8px;background:#1f4fd6;color:#fff;display:flex;align-items:center;justify-content:center}.company h1{font-size:16px;margin:0}.company p{font-size:8.5px;color:#667085;margin:2px 0}.doc-title{text-align:right}.doc-title .type{font-size:8px;text-transform:uppercase;color:#667085;letter-spacing:1px}.doc-title h2{font-size:18px;color:#1f4fd6;margin:3px 0}.client-data{border:1px solid #d0d5dd;border-radius:7px;padding:8px;margin:10px 0;break-inside:avoid}.client-data-title{font-size:9px;font-weight:700;text-transform:uppercase;color:#1f4fd6;margin-bottom:5px}.client-data-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:2px 12px}.client-data p{margin:2px 0;font-size:8.5px}.client-subtitle{font-size:8px;font-weight:700;margin-top:6px;color:#475467}.summary{display:grid;grid-template-columns:repeat(4,1fr);gap:7px;margin:10px 0}.summary div{border:1px solid #e4e9f2;border-radius:7px;padding:7px;background:#f8fafc}.summary small{display:block;color:#667085;text-transform:uppercase;font-size:7.5px}.summary b{font-size:10px}.extra{font-size:8.5px;color:#475467;margin:5px 0}.floor-block{break-before:page;padding-top:3px}.floor-block:first-of-type{break-before:auto}.floor-title{display:flex;align-items:end;gap:8px;border-bottom:2px solid #1f4fd6;margin:8px 0}.floor-title span{font-size:7.5px;color:#667085}.floor-title h2{font-size:15px;margin:0 0 4px}.vertical-card{border:1px solid #d0d5dd;border-radius:7px;margin:7px 0;break-inside:avoid;overflow:hidden}.vertical-card-head{display:flex;justify-content:space-between;background:#f2f5fa;padding:7px 9px}.vertical-card-head h3{margin:1px 0;font-size:11px}.vertical-card-head small{font-size:7px;color:#667085}.vertical-meta{text-align:right;font-size:8px;color:#475467}.vertical-line{display:grid;grid-template-columns:12px 1fr 100px;gap:7px;align-items:center;border-top:1px solid #edf0f5;padding:6px 9px}.vertical-dot{width:6px;height:6px;background:#1f4fd6;border-radius:50%}.vertical-line b,.vertical-line small{display:block}.vertical-line small{font-size:8px;color:#667085;margin-top:2px}.vertical-line strong{text-align:right}.empty-line{padding:8px;color:#98a2b3}.footer{margin-top:10px;padding-top:6px;border-top:1px solid #e4e9f2;color:#98a2b3;font-size:7.5px;display:flex;justify-content:space-between}@media print{body{-webkit-print-color-adjust:exact;print-color-adjust:exact}}</style></head><body><div class="doc">${printBrandHeader('Esquema Vertical','Documento técnico')}<div class="summary"><div><small>Obra</small><b>${esc(obraEletricaAtual.nome)}</b></div><div><small>Cliente</small><b>${esc(cli?.nome||'-')}</b></div><div><small>Carga instalada</small><b>${oeFmtKw(d.carga_instalada)}</b></div><div><small>Demanda</small><b>${oeFmtKw(d.demanda)}</b></div></div>${clientePDFHTML(cli)}${extras.length?`<div class="extra">${extras.map(esc).join(' &nbsp; | &nbsp; ')}</div>`:''}${esquemaPavimentosHTML()}<div class="footer"><span>${esc(empresa?.rodape_documentos||'Documento emitido pelo Core Orça')}</span><span>Emitido em ${new Date().toLocaleString('pt-BR')}</span></div></div><script>onload=()=>setTimeout(()=>print(),350)<\/script></body></html>`);w.document.close()}

/* Lista de materiais V045 — editor em planilha, tópicos reordenáveis e PDF institucional sem valores */
async function openListaMaterialForm(id=null){let lista=null;if(id){const x=await getListaMaterial(id);if(!x)return;lista=x.lista;listaMaterialItens=x.itens.map(i=>({...i}));listaMaterialTopicos=(lista.topicos?.length?lista.topicos:[...new Set(listaMaterialItens.map(i=>i.topico||'Geral'))]);}else{listaMaterialItens=[];listaMaterialTopicos=['Geral']}openModal(lista?.id?'Editar Lista de Materiais':'Nova Lista de Materiais',`<form id="lista-material-form"><div class="form-grid"><label>Nome da lista*<input id="lm-nome" required value="${esc(lista?.nome||'')}"></label><label>Data*<input id="lm-data" type="date" required value="${lista?.data_lista||today()}"></label><label>Cliente<select id="lm-cliente" data-no-search="true"><option value="">Selecione...</option>${clientes.map(c=>`<option value="${c.id}" ${c.id===lista?.cliente_id?'selected':''}>${esc(c.nome)}</option>`).join('')}</select></label><label>Responsável<input id="lm-responsavel" value="${esc(lista?.responsavel_emissao||currentUserProfile?.nome||'')}"></label><label>Vínculo<select id="lm-tipo" data-no-search="true"><option value="sem_vinculo">Sem vínculo</option><option value="orcamento">Orçamento</option><option value="recibo">Recibo</option><option value="ordem_servico">OS</option></select></label><label class="span-2">Observações<textarea id="lm-obs">${esc(lista?.observacoes||'')}</textarea></label></div><div class="lm-topic-toolbar"><button type="button" class="btn secondary" id="lm-add-topic">+ Tópico</button><select id="lm-material"><option value="">Selecione...</option>${materiais.filter(m=>m.id).map(m=>`<option value="${m.id}">${esc(m.codigo||'')} — ${esc(m.nome)}</option>`).join('')}</select><input id="lm-qtd" type="number" min="0.01" step="0.01" value="1"><label class="lm-obs-add">Observações<textarea id="lm-item-obs" placeholder="Marca, modelo, cor, observações..."></textarea></label><button type="button" class="btn primary" id="lm-add">Adicionar</button></div><div class="lm-sheet"><div class="lm-sheet-head"><span>Material / especificação</span><span>Quantidade</span><span>Tópico</span><span>Ações</span></div><div id="lm-topic-editor" class="lm-topic-editor"></div></div><div class="modal-actions"><button type="button" class="btn secondary" onclick="closeModal()">Cancelar</button><button class="btn primary">Salvar Lista</button></div></form>`);if(lista)$('lm-tipo').value=lista.tipo_vinculo||'sem_vinculo';const refreshTopics=()=>renderListaMaterialItens();$('lm-add-topic').onclick=()=>{const n=prompt('Nome do tópico:');if(n?.trim()&&!listaMaterialTopicos.includes(n.trim())){listaMaterialTopicos.push(n.trim());refreshTopics();renderListaMaterialItens()}};$('lm-add').onclick=()=>{const materialId=String($('lm-material')?.value||'');const m=materiais.find(x=>String(x.id)===materialId);if(!m)return toast('Selecione um material.');const obs=[$('lm-item-obs').value.trim(),m.fabricante?`Marca: ${m.fabricante}`:''].filter(Boolean).join('\n');listaMaterialItens.push({material_id:m.id,codigo:m.codigo,descricao:m.nome,categoria:m.categoria,unidade:m.unidade||'un',quantidade:Number($('lm-qtd').value)||1,valor_unitario:0,observacoes:obs,topico:listaMaterialTopicos[0]||'Geral'});renderListaMaterialItens()};$('lista-material-form').onsubmit=e=>saveListaMaterial(e,id);renderListaMaterialItens();enhanceSearchableSelect($('lm-material'))}
function renderListaMaterialItens(){const box=$("lm-topic-editor");if(!box)return;box.innerHTML=listaMaterialTopicos.map((t,ti)=>`<section class="lm-topic" draggable="true" data-topic="${esc(t)}"><header><span class="drag-handle">⋮⋮</span><b>${esc(t)}</b><div><button type="button" class="action-btn" onclick="moveLmTopic(${ti},-1)">↑</button><button type="button" class="action-btn" onclick="moveLmTopic(${ti},1)">↓</button></div></header><div class="lm-topic-items">${listaMaterialItens.filter(i=>(i.topico||"Geral")===t).map(i=>{const n=listaMaterialItens.indexOf(i);return `<div class="lm-item" draggable="true" data-index="${n}"><span class="drag-handle">⋮⋮</span><div class="lm-item-desc"><b>${esc(i.descricao)}</b>${i.observacoes?`<small>${richDisplay(i.observacoes)}</small>`:""}</div><span>${Number(i.quantidade).toLocaleString("pt-BR")} ${esc(i.unidade)}</span><div class="actions"><button type="button" class="action-btn" onclick="editListaMaterialItem(${n})">Editar</button><button type="button" class="action-btn danger" onclick="removeListaMaterialItem(${n})">Excluir</button></div>${lmEditingIndex===n?`<div class="lm-inline-editor"><label>Quantidade<input id="lm-eq-${n}" type="number" min="0.01" step="0.01" value="${Number(i.quantidade)||1}"></label><label>Unidade<input id="lm-eu-${n}" value="${esc(i.unidade||"un")}"></label><label class="span-2">Observações<textarea id="lm-eo-${n}">${esc(i.observacoes||"")}</textarea></label><div class="span-2 actions"><button type="button" class="btn primary" onclick="saveListaMaterialItemEdit(${n})">Aplicar</button><button type="button" class="btn secondary" onclick="lmEditingIndex=-1;renderListaMaterialItens()">Cancelar</button></div></div>`:""}</div>`}).join("")||'<p class="muted">Arraste materiais para este tópico.</p>'}</div></section>`).join("");enableLmDrag();setTimeout(()=>enhanceObservationEditors(box),0)}
function editListaMaterialItem(n){lmEditingIndex=n;renderListaMaterialItens();setTimeout(()=>enhanceObservationEditors($("lm-topic-editor")),0)}
function saveListaMaterialItemEdit(n){const i=listaMaterialItens[n];if(!i)return;i.quantidade=Number($("lm-eq-"+n)?.value)||1;i.unidade=$("lm-eu-"+n)?.value.trim()||"un";i.observacoes=$("lm-eo-"+n)?.value||"";lmEditingIndex=-1;renderListaMaterialItens()}
function moveLmTopic(i,d){const j=i+d;if(j<0||j>=listaMaterialTopicos.length)return;[listaMaterialTopicos[i],listaMaterialTopicos[j]]=[listaMaterialTopicos[j],listaMaterialTopicos[i]];renderListaMaterialItens()}
function removeListaMaterialItem(n){listaMaterialItens.splice(n,1);renderListaMaterialItens()}
function updateListaMaterialTotal(){return 0}
function enableLmDrag(){let src=null;document.querySelectorAll('.lm-item').forEach(el=>{el.ondragstart=e=>{src=Number(el.dataset.index);e.stopPropagation()};el.ondragover=e=>e.preventDefault();el.ondrop=e=>{e.preventDefault();e.stopPropagation();const dst=Number(el.dataset.index);if(src==null||src===dst)return;const targetTopic=listaMaterialItens[dst]?.topico||'Geral';const [x]=listaMaterialItens.splice(src,1);x.topico=targetTopic;let pos=dst;if(src<dst)pos--;listaMaterialItens.splice(Math.max(0,pos),0,x);renderListaMaterialItens()}});document.querySelectorAll('.lm-topic').forEach(el=>{el.ondragover=e=>{e.preventDefault();el.classList.add('drag-over')};el.ondragleave=()=>el.classList.remove('drag-over');el.ondrop=e=>{if(src==null)return;e.preventDefault();el.classList.remove('drag-over');const x=listaMaterialItens[src];if(x)x.topico=el.dataset.topic;renderListaMaterialItens()}})}
async function saveListaMaterial(e,id){e.preventDefault();if(!listaMaterialItens.length)return toast('Adicione pelo menos um material.');const header={nome:$('lm-nome').value.trim(),data_lista:$('lm-data').value,cliente_id:$('lm-cliente').value||null,responsavel_emissao:$('lm-responsavel').value.trim()||null,tipo_vinculo:$('lm-tipo').value,observacoes:$('lm-obs').value.trim()||null,total:0,topicos:listaMaterialTopicos,updated_at:new Date().toISOString()};let listaId=id;if(id){let r=await sb.from('listas_materiais').update(header).eq('id',id);if(r.error)return toast(r.error.message);await sb.from('lista_materiais_itens').delete().eq('lista_id',id)}else{const r=await sb.from('listas_materiais').insert(header).select().single();if(r.error)return toast(r.error.message);listaId=r.data.id}const payload=listaMaterialItens.map((i,n)=>({lista_id:listaId,material_id:i.material_id||null,codigo:i.codigo||null,descricao:i.descricao,categoria:i.categoria||null,unidade:i.unidade||'un',quantidade:Number(i.quantidade),valor_unitario:0,observacoes:i.observacoes||null,ordem:n,topico:i.topico||'Geral'}));const ins=await sb.from('lista_materiais_itens').insert(payload);if(ins.error)return toast(ins.error.message);closeModal();await loadListasMateriais();renderListasMateriais();toast('Lista salva')}
async function viewListaMaterial(id){const x=await getListaMaterial(id);if(!x)return;const groups=x.lista.topicos?.length?x.lista.topicos:[...new Set(x.itens.map(i=>i.topico||'Geral'))];openModal(x.lista.nome,`${groups.map(t=>`<section class="lm-view-topic"><h4>${esc(t)}</h4>${x.itens.filter(i=>(i.topico||'Geral')===t).map(i=>`<div class="lm-view-row"><div><b>${esc(i.descricao)}</b>${i.observacoes?`<small>${richDisplay(i.observacoes)}</small>`:''}</div><span>${Number(i.quantidade).toLocaleString('pt-BR')} ${esc(i.unidade)}</span></div>`).join('')}</section>`).join('')}<div class="modal-actions"><button class="btn secondary" onclick="closeModal()">Fechar</button><button class="btn primary" onclick="printListaMaterial('${id}')">PDF / Imprimir</button></div>`)}
async function printListaMaterial(id){const x=await getListaMaterial(id);if(!x)return;const l=x.lista,topics=(l.topicos?.length?l.topicos:[...new Set(x.itens.map(i=>i.topico||'Geral'))]),cli=l.clientes,w=window.open('','_blank');if(!w)return toast('Permita pop-ups para gerar o PDF.');w.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>${esc(l.nome)}</title><style>@page{size:A4;margin:12mm 13mm 14mm}*{box-sizing:border-box}body{font:10px Arial;color:#172033;margin:0}.doc{max-width:190mm;margin:auto}.top{display:flex;justify-content:space-between;align-items:flex-start;padding-bottom:11px;border-bottom:2px solid #1f4fd6}.brand{display:flex;gap:10px;align-items:center}.brand-logo{max-width:145px;max-height:52px}.brand-mark{width:40px;height:40px;border-radius:9px;background:#1f4fd6;color:#fff;display:flex;align-items:center;justify-content:center}.company h1{font-size:16px;margin:0}.company p{font-size:8.5px;color:#667085;margin:2px 0}.doc-title{text-align:right}.doc-title .type{font-size:8px;text-transform:uppercase;color:#667085;letter-spacing:1px}.doc-title h2{font-size:18px;color:#1f4fd6;margin:3px 0}.client-data{border:1px solid #d0d5dd;border-radius:7px;padding:8px;margin:10px 0;break-inside:avoid}.client-data-title{font-size:9px;font-weight:700;text-transform:uppercase;color:#1f4fd6;margin-bottom:5px}.client-data-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:2px 12px}.client-data p{margin:2px 0;font-size:8.5px}.client-subtitle{font-size:8px;font-weight:700;margin-top:6px;color:#475467}.info{display:grid;grid-template-columns:2fr 1fr 1fr;gap:7px;margin:10px 0}.info div{border:1px solid #e4e9f2;border-radius:7px;padding:7px;background:#f8fafc}.info small{display:block;color:#667085;text-transform:uppercase;font-size:7.5px}.topic{margin:12px 0;break-inside:auto}.topic h2{font-size:11px;text-transform:uppercase;letter-spacing:.5px;color:#344054;background:#f2f5fa;border-left:3px solid #1f4fd6;padding:6px 8px;margin:0;break-after:avoid}.table{border:1px solid #d0d5dd;border-radius:6px;overflow:hidden}.thead,.row{display:grid;grid-template-columns:1fr 125px}.thead{background:#fff;font-size:8px;text-transform:uppercase;text-align:center;font-weight:700;border-bottom:1px solid #98a2b3}.thead span{padding:5px}.thead span+span,.qty{border-left:1px solid #d0d5dd}.row{min-height:42px;border-bottom:1px solid #e4e7ec;break-inside:avoid;page-break-inside:avoid}.row:last-child{border-bottom:0}.mat{padding:7px 9px}.mat b{display:block;font-size:10.5px}.mat small{display:block;font-size:8.5px;font-style:italic;line-height:1.35;margin-top:3px;color:#344054}.qty{display:flex;align-items:center;justify-content:center;padding:7px;text-align:center}.notes{margin-top:12px;border-top:1px solid #e4e9f2;padding-top:7px;color:#667085;font-size:8.5px}.footer{margin-top:16px;padding-top:7px;border-top:1px solid #e4e9f2;color:#98a2b3;font-size:7.5px;display:flex;justify-content:space-between;break-inside:avoid}@media print{body{-webkit-print-color-adjust:exact;print-color-adjust:exact}.topic{orphans:2;widows:2}}</style></head><body><div class="doc">${printBrandHeader(l.nome,'Lista de Materiais')}${clientePDFHTML(cli)}<div class="info"><div><small>Cliente</small><b>${esc(cli?.nome||'-')}</b></div><div><small>Data</small><b>${new Date(l.data_lista+'T12:00:00').toLocaleDateString('pt-BR')}</b></div><div><small>Responsável</small><b>${esc(l.responsavel_emissao||'-')}</b></div></div>${topics.map(t=>{const its=x.itens.filter(i=>(i.topico||'Geral')===t);if(!its.length)return '';return `<section class="topic"><h2>${esc(t)}</h2><div class="table"><div class="thead"><span>Material</span><span>Quantidade</span></div>${its.map(i=>`<div class="row"><div class="mat"><b>${esc(i.descricao)}</b>${i.observacoes?`<small>${richDisplay(i.observacoes)}</small>`:''}</div><div class="qty">${Number(i.quantidade).toLocaleString('pt-BR')} ${esc(i.unidade)}</div></div>`).join('')}</div></section>`}).join('')}${l.observacoes?`<div class="notes"><b>Observações:</b> ${esc(l.observacoes)}</div>`:''}<div class="footer"><span>${esc(empresa?.rodape_documentos||'Documento emitido pelo Core Orça')}</span><span>Emitido em ${new Date().toLocaleString('pt-BR')}</span></div></div><script>onload=()=>{const imgs=[...document.images];Promise.all(imgs.map(i=>i.complete?Promise.resolve():new Promise(r=>{i.onload=r;i.onerror=r}))).then(()=>setTimeout(()=>print(),400))}<\/script></body></html>`);w.document.close()}

// Esqueci minha senha: solicita senha temporária por e-mail pela Edge Function.
async function solicitarSenhaTemporaria(){const email=$('login-email').value.trim().toLowerCase();if(!email){$('login-error').textContent='Informe seu e-mail.';return}const {data,error}=await sb.functions.invoke('core-orca-admin-users',{body:{action:'forgot-password-email',email}});$('login-error').textContent=error?'Não foi possível processar agora.':(data?.message||'Se o e-mail estiver cadastrado, uma senha temporária será enviada.')}
