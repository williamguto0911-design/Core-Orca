...
function sanitizeRichText(html){const t=document.createElement("template");t.innerHTML=String(html||"");const ok=new Set(["B","STRONG","I","EM","U","BR","DIV","P","SPAN"]);[...t.content.querySelectorAll("*")].forEach(el=>{if(!ok.has(el.tagName)){el.replaceWith(...el.childNodes);return} [...el.attributes].forEach(a=>{if(!(el.tagName==="SPAN"&&a.name==="style"&&/color\s*:\s*(red|black|#f?00|#ff0000|#000000|rgb\(255,\s*0,\s*0\)|rgb\(0,\s*0,\s*0\))/i.test(a.value)))el.removeAttribute(a.name)})});return t.innerHTML}
...
function adicionarComponente(c0=null){
 ...
 $('oe-comp-form').onsubmit=async e=>{
  ...
  let r=c0?await sb.from('obra_eletrica_componentes').update(payload).eq('id',c0.id).select().single():await sb.from('obra_eletrica_componentes').insert({...payload,empresa_id:empresaId,obra_id:obraId,ordem:obraEletricaComponentes.length}).select().single();
