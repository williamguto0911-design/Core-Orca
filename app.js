...
function listaMaterialForm(lista=null,itens=[]){
 ...
 <label>Vínculo<select id="lm-tipo"><option value="sem_vinculo" ${tipo==="sem_vinculo"?"selected":""}>Sem vínculo</option><option value="orcamento" ${tipo==="orcamento"?"selected":""}>Orçamento</option><option value="recibo" ${tipo==="recibo"?"selected":""}>Recibo</option><option value="os" ${tipo==="os"?"selected":""}>Ordem de Serviço (OS)</option></select></label>
 ...
 const syncVinculo=()=>{const t=$("lm-tipo").value;$("lm-orc-wrap").classList.toggle("hidden",t!=="orcamento");$("lm-rec-wrap").classList.toggle("hidden",t!=="recibo");$("lm-os-wrap").classList.toggle("hidden",t!=="os")};
