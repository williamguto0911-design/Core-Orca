...
function renderUfCustomList(){const el=$("uf-custom-list");if(!el)return;el.innerHTML=ufCustomSymbols.map(s=>`<button draggable="true" data-uf-custom="${s.id}">${ufSvg(s.svg)}<span>${esc(s.nome)}</span></button>`).join("");el.querySelectorAll("[data-uf-custom]").forEach(b=>b.ondragstart=e=>e.dataTransfer.setData("text/uf-custom",b.dataset.ufCustom))}
...
function addUfNode(type,x,y,custom=null){const d=custom?{label:custom.nome,svg:custom.svg}:UF[type];if(!d)return;unifilarDoc.elementos.push({id:ufId(),type:custom?"custom":type,custom_symbol_id:custom?.id||null,custom_name:custom?.nome||null,custom_svg:custom?.svg||null,x:Math.max(10,Math.round(x/20)*20),y:Math.max(10,Math.round(y/20)*20),label:d.label,corrente:["idr","disjuntor"].includes(type)?40:null,sensibilidade:type==="idr"?30:null,polos:type==="idr"?2:null,potencia:type==="circuito"?1000:null,tensao:type==="circuito"?220:null,fp:1,material_id:null});drawUnifilar()}
...
function drawUnifilar(){const box=$("unifilar-nodes"),svg=$("unifilar-lines");if(!box||!svg)return;box.innerHTML=unifilarDoc.elementos.map(n=>{const d=ufDef(n);return `<div class="uf-node uf-${n.type} ${unifilarSelected===n.id?"selected":""}" data-uf-id="${n.id}" style="left:${n.x}px;top:${n.y}px"><i class="uf-port in" data-port="in"></i><div class="uf-symbol">${ufSvg(d.svg)}</div><div class="uf-label">${esc(n.label||d.label)}</div><div class="uf-sub">${esc(ufSub(n))}</div><i class="uf-port out" data-port="out"></i></div>`}).join("");drawUfLines();box.querySelectorAll(".uf-node").forEach(el=>{el.onclick=e=>{e.stopPropagation();const id=el.dataset.ufId;if(e.target.closest(".uf-port")?.dataset.port==="out"){unifilarConnectFrom=id;toast("Selecione a entrada do componente de destino");return}if(e.target.closest(".uf-port")?.dataset.port==="in"&&unifilarConnectFrom&&unifilarConnectFrom!==id){if(!unifilarDoc.conexoes.some(c=>c.from===unifilarConnectFrom&&c.to===id))unifilarDoc.conexoes.push({id:ufId(),from:unifilarConnectFrom,to:id,condutor:"fase"});unifilarConnectFrom=null;drawUnifilar();return}unifilarSelected=id;drawUnifilar()};el.onpointerdown=e=>{if(e.target.closest(".uf-port"))return;const n=unifilarDoc.elementos.find(x=>x.id===el.dataset.ufId),r=$("unifilar-canvas").getBoundingClientRect(),dx=e.clientX-r.left+$("unifilar-canvas").scrollLeft-n.x,dy=e.clientY-r.top+$("unifilar-canvas").scrollTop-n.y;el.setPointerCapture(e.pointerId);el.onpointermove=ev=>{n.x=Math.max(0,Math.round((ev.clientX-r.left+$("unifilar-canvas").scrollLeft-dx)/20)*20);n.y=Math.max(0,Math.round((ev.clientY-r.top+$("unifilar-canvas").scrollTop-dy)/20)*20);el.style.left=n.x+"px";el.style.top=n.y+"px";drawUfLines()};el.onpointerup=()=>el.onpointermove=null}});drawUfProperties();calcUfSummary()}
function drawUfLines(){const by=Object.fromEntries(unifilarDoc.elementos.map(n=>[n.id,n]));$("unifilar-lines").innerHTML=unifilarDoc.conexoes.map(c=>{const a=by[c.from],b=by[c.to];if(!a||!b)return"";const x1=a.x+60,y1=a.y+86,x2=b.x+60,y2=b.y,ym=Math.round((y1+y2)/2/20)*20;const color=c.condutor==="neutro"?"#13b5ea":c.condutor==="terra"?"#36d657":c.condutor==="fase2"?"#ef4444":"#d7dee7";return `<path d="M ${x1} ${y1} V ${ym} H ${x2} V ${y2}" fill="none" stroke="${color}" stroke-width="2"/><circle cx="${x1}" cy="${ym}" r="3" fill="${color}"/>`}).join("")}
