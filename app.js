...
async function printListaMaterial(id){const x=await getListaMaterial(id);if(!x)return;const l=x.lista;const vinc=l.tipo_vinculo==="orcamento"?`Orçamento ${l.orcamentos?.numero||""}`:l.tipo_vinculo==="recibo"?`Recibo ${l.recibos?.numero||""}`:l.tipo_vinculo==="ordem_servico"?`OS ${l.ordens_servico?.numero||""}`:"Sem vínculo";const itens=x.itens.map(i=>({tipo:"material",descricao:`${i.codigo?i.codigo+" — ":""}${i.descricao}${i.observacoes?" — "+i.observacoes:""}`,quantidade:i.quantidade,valor_unitario:i.valor_unitario}));printDocument(`Lista de Materiais — ${l.nome}`,`<p><b>Cliente:</b> ${esc(l.clientes?.nome||"-")}</p><p><b>Data:</b> ${new Date(l.data_lista+"T12:00:00").toLocaleDateString("pt-BR")}</p><p><b>Vínculo:</b> ${esc(vinc)}</p><p><b>Responsável pela emissão:</b> ${esc(l.responsavel_emissao||"-")}</p><p><b>Itens:</b> ${x.itens.length}</p>`,itens,`<p class="total">Total geral: ${money(l.total)}</p>${l.observacoes?`<div class="section"><div class="section-title">Observações</div><p>${esc(l.observacoes)}</p></div>`:""}`)}
