...
async function refreshAll(){
  ...
  if(new URLSearchParams(location.search).get("view")==="unifilar" && companyModules?.unifilar!==false){navigate("unifilar");document.body.classList.add("unifilar-standalone");}
}
...
$("menu-btn").onclick=()=>$("sidebar").classList.toggle("open");document.querySelectorAll("[data-page]").forEach(b=>b.onclick=()=>{if(b.dataset.page==="unifilar"&&new URLSearchParams(location.search).get("view")!=="unifilar"){const u=new URL(location.href);u.searchParams.set("view","unifilar");window.open(u.toString(),"_blank","noopener");return}navigate(b.dataset.page)});
