...
function reciboForm(){
 editorItens=[];let pagamentos=[{data:today(),valor:0,status:"pendente"}];
 ...
 const ajustarParcelaUnica=()=>{if(pagamentos.length===1){pagamentos[0].valor=Math.round(editorItens.reduce((s,i)=>s+i.quantidade*i.valor_unitario,0)*100)/100;draw()}};
 $("f-vinc-tipo").onchange=async()=>{const tipo=$("f-vinc-tipo").value,sel=$("f-vinc-id");editorItens=[];renderEditorItens();if(tipo==="sem_vinculo"){sel.innerHTML='<option value="">Sem vínculo</option>';sel.disabled=true;ajustarParcelaUnica();return}sel.disabled=false;sel.innerHTML=reciboVinculoOptions(tipo);enhanceAllSelects($("modal-body"));};
 $("f-vinc-id").onchange=async()=>{const tipo=$("f-vinc-tipo").value,id=$("f-vinc-id").value;if(!id)return;const cli=reciboVinculoCliente(tipo,id);if(cli)$("f-cliente").value=cli;const n=await carregarItensVinculoRecibo(tipo,id);if(n>=0){ajustarParcelaUnica();toast(`${n} item(ns) carregado(s) automaticamente.`)}};
