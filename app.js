...
async function loadListasMateriais(){const {data,error}=await sb.from("listas_materiais").select("*,clientes(nome),orcamentos(numero),recibos(numero),ordens_servico(numero),lista_materiais_itens(id)").order("created_at",{ascending:false});if(error){listasMateriais=[];console.warn("Listas de materiais:",error.message);return}listasMateriais=data||[]}
