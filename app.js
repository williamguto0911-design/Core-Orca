...
let clientes=[], materiais=[], servicos=[], orcamentos=[], ordens=[], currentPage="dashboard";
let editorItens=[];
...
async function refreshAll(){
  await Promise.all([loadClientes(),loadMateriais(),loadServicos(),loadOrcamentos(),loadOrdens()]);
...
function navigate(page){
  ...
  $("page-title").textContent={dashboard:"Dashboard",clientes:"Clientes",materiais:"Materiais",servicos:"Serviços",estoque:"Estoque",orcamentos:"Orçamentos",os:"Ordens de Serviço"}[page];
...
async function concluirOS(id){
 ...
 closeModal();await Promise.all([loadMateriais(),loadOrdens()]);renderOS();renderDashboard();toast(data||"OS concluída.");
}
