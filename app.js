...
function adicionarComponente(c0=null){
 ...
 $('oe-comp-form').onsubmit=async e=>{
  e.preventDefault();
  const sid=String($('oec-struct')?.value||'').trim();
  const mid=String($('oec-material')?.value||'').trim();
  const st=obraEletricaEstruturas.find(x=>String(x.id)===sid);
  if(!sid||!st)return toast('Selecione um local / quadro válido.');
  if(!mid||!materiais.some(x=>String(x.id)===mid))return toast('Selecione um material válido do banco.');
  ...
  if(!payload.pavimento_id)return toast('O quadro selecionado não possui pavimento válido. Edite o quadro e selecione o pavimento.');
  let r=c0?await sb.from('obra_eletrica_componentes').update(payload).eq('id',c0.id).select().single():await sb.from('obra_eletrica_componentes').insert({...payload,empresa_id:currentUserProfile.empresa_id,obra_id:obraEletricaAtual.id,ordem:obraEletricaComponentes.length}).select().single();
