...
function enableLmDrag(){let src=null;document.querySelectorAll('.lm-item').forEach(el=>{el.ondragstart=()=>src=Number(el.dataset.index);el.ondragover=e=>e.preventDefault();el.ondrop=e=>{e.preventDefault();const dst=Number(el.dataset.index);if(src==null||src===dst)return;const [x]=listaMaterialItens.splice(src,1);listaMaterialItens.splice(dst,0,x);renderListaMaterialItens()}});document.querySelectorAll('.lm-topic').forEach(el=>{el.ondragover=e=>e.preventDefault();el.ondrop=e=>{if(src==null)return;e.preventDefault();listaMaterialItens[src].topico=el.dataset.topic;renderListaMaterialItens()}})}
